#include <navit_bt_navigator/way_points_navigator_action_server_node.hpp>

using namespace BT;
using namespace navit_bt_nodes;


void WayPointsNavigatorActionServer::executeCB(const navit_msgs::WayPointsNavigatorGoalConstPtr &goal)
{
    ...
    while (ros::ok() && future_status == NodeStatus::RUNNING && start_cmd)
    {
        ...
        if (future_status == NodeStatus::FAILURE)
            as_.setAborted(navit_msgs::WayPointsNavigatorResult(), "Aborting on the goal because the BT node future_status is FAILURE");
        else if (future_status == NodeStatus::SUCCESS)
            as_.setSucceeded(navit_msgs::WayPointsNavigatorResult(), "Goal reached.");
    ...
    //         as_.setAborted(navit_msgs::WayPointsNavigatorResult(), "Aborting on the goal because the BT node status is FAILURE");
...
{
    ros::init(argc, argv, "way_points_navigator");
    
    WayPointsNavigatorActionServer way_points_navigator("way_points_navigator");
    ros::spin();
