...
#include <navit_msgs/WayPointsNavigatorAction.h>
...
class WayPointsNavigatorActionServer
{
protected:

  ros::NodeHandle nh_;
  actionlib::SimpleActionServer<navit_msgs::WayPointsNavigatorAction> as_; // NodeHandle instance must be created before this line. Otherwise strange error occurs.
  std::string action_name_;
  // create messages that are used to published feedback/result
  navit_msgs::WayPointsNavigatorFeedback feedback_;
  navit_msgs::WayPointsNavigatorResult result_;

public:

    WayPointsNavigatorActionServer(std::string name):
        as_(nh_, name, boost::bind(&WayPointsNavigatorActionServer::executeCB, this, _1), false),
        action_name_(name)
    {
    ...
    ~WayPointsNavigatorActionServer(void)
    {
        // halt all BT nodes
        //
    }
    ...
    void executeCB(const navit_msgs::WayPointsNavigatorGoalConstPtr &goal);
