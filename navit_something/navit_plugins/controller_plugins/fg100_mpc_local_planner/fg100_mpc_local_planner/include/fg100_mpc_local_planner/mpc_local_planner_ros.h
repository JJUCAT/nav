...
#include <teb_local_planner/goal_functions.h>
#include <teb_local_planner/odometry_helper_ros.h>
#include <navit_core/base_controller.h>

// mpc_local_planner related classes
#include <fg100_mpc_local_planner/controller.h>
#include <fg100_mpc_local_planner/utils/publisher.h>
 ...
 */
class MpcLocalPlannerROS : public navit_core::Controller 
{
 ...
 public:
                            ...
                            const geometry_msgs::Twist& current_vel) override
    {
        ...
        if ( computeVelocityCommands(cmd_vel) )
            ...
            throw std::runtime_error("no feasible cmd vel found");
    }
    ...
    bool setPlan(const nav_msgs::Path& plan) override { return setPlan(plan.poses);}
    ...
    bool setSpeedLimit(const double&) override {return false;}
     ...
     * @param costmap_ros pointer to an intialized instance of navit_costmap::Costmap2dROS
     * @return Robot footprint model used for optimization
     */
    static RobotFootprintModelPtr getRobotFootprintFromParamServer(const ros::NodeHandle& nh, navit_costmap::Costmap2DROS* costmap_ros = nullptr);

    /**
     * @brief Get the current robot footprint/contour model
     * @param costmap_ros reference to an intialized instance of navit_costmap::Costmap2dROS
     * @return Robot footprint model used for optimization
     */
    static RobotFootprintModelPtr getRobotFootprintFromCostmap2d(navit_costmap::Costmap2DROS& costmap_ros);

    /**
     * @brief Set the footprint from the given XmlRpcValue.
     * @remarks This method is copied from navit_costmap/footprint.h, since it is not declared public in all ros distros
 ...
 protected:
     ...
     *
     * The method replaces transformGlobalPlan as defined in teb_local_planner/goal_functions.h
    ...
    bool transformGlobalPlan(const tf2_ros::Buffer& tf, const std::vector<geometry_msgs::PoseStamped>& global_plan,
                             const geometry_msgs::PoseStamped& global_pose, const navit_costmap::Costmap2D& costmap, const std::string& global_frame,
 ...
 private:
    ...
    navit_costmap::Costmap2DROS* _costmap_ros;  //!< Pointer to the costmap ros wrapper, received from the navigation stack
    navit_costmap::Costmap2D* _costmap;         //!< Pointer to the 2d costmap (obtained from the costmap ros wrapper)
    tf2_ros::Buffer* _tf;                    //!< pointer to tf buffer

    // internal objects
    fg100_mpc_local_planner::Controller _controller;
    ObstContainer _obstacles;  //!< Obstacle vector that should be considered during local trajectory optimization
    Publisher _publisher;
    std::shared_ptr<teb_local_planner::CostmapModel> _costmap_model;
    ...
    teb_local_planner::OdometryHelperRos _odom_helper;  //!< Provides an interface to receive the current velocity from the robot
