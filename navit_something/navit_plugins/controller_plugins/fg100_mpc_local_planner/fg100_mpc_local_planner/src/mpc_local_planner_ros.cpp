...
#include <fg100_mpc_local_planner/mpc_local_planner_ros.h>
...
PLUGINLIB_EXPORT_CLASS(fg100_mpc_local_planner::MpcLocalPlannerROS, navit_core::Controller)
...
void MpcLocalPlannerROS::initialize(std::string name, tf2_ros::Buffer* tf, navit_costmap::Costmap2DROS* costmap_ros)
{
    ...
    if (!_initialized)
    {
        ...
        _costmap_model = std::make_shared<teb_local_planner::CostmapModel>(*_costmap);
        ...
        navit_costmap::calculateMinAndMaxDistances(_footprint_spec, _robot_inscribed_radius, _robot_circumscribed_radius);
...
void MpcLocalPlannerROS::updateObstacleContainerWithCostmap()
{
    ...
    if (_params.include_costmap_obstacles)
    {
        ...
        for (unsigned int i = 0; i < _costmap->getSizeInCellsX() - 1; ++i)
        {
            ...
            {
                if (_costmap->getCost(i, j) == navit_costmap::LETHAL_OBSTACLE)
                {
                    Eigen::Vector2d obs;
...
bool MpcLocalPlannerROS::transformGlobalPlan(const tf2_ros::Buffer& tf, const std::vector<geometry_msgs::PoseStamped>& global_plan,
                                             const geometry_msgs::PoseStamped& global_pose, const navit_costmap::Costmap2D& costmap,
...
{
    // this method is a slightly modified version of teb_local_planner/goal_functions.h
...
teb_local_planner::RobotFootprintModelPtr MpcLocalPlannerROS::getRobotFootprintFromParamServer(const ros::NodeHandle& nh,
                                                                                               navit_costmap::Costmap2DROS* costmap_ros)
{
    ...
    // from navit_costmap
    if (model_name.compare("navit_costmap") == 0)
    {
        if (!costmap_ros)
        {
        ...
        ROS_INFO("Footprint model loaded from navit_costmap for trajectory optimization.");
...
teb_local_planner::RobotFootprintModelPtr MpcLocalPlannerROS::getRobotFootprintFromCostmap2d(navit_costmap::Costmap2DROS& costmap_ros)
{
