...
#include <fg100_mpc_local_planner/optimal_control/fd_collocation_se2.h>
#include <fg100_mpc_local_planner/optimal_control/final_state_conditions_se2.h>
#include <fg100_mpc_local_planner/optimal_control/finite_differences_variable_grid_se2.h>
#include <fg100_mpc_local_planner/optimal_control/min_time_via_points_cost.h>
#include <fg100_mpc_local_planner/optimal_control/quadratic_cost_se2.h>
#include <fg100_mpc_local_planner/optimal_control/stage_inequality_se2.h>
#include <fg100_mpc_local_planner/systems/kinematic_bicycle_model.h>
#include <fg100_mpc_local_planner/systems/simple_car.h>
#include <fg100_mpc_local_planner/systems/unicycle_robot.h>
#include <fg100_mpc_local_planner/systems/simple_car_delay.h>
#include <fg100_mpc_local_planner/utils/time_series_se2.h>
...
corbo::DiscretizationGridInterface::Ptr Controller::configureGrid(const ros::NodeHandle& nh)
{
    ...
    if (grid_type == "fd_grid")
    {
        ...
        if(_robot_type == "simple_car_delay")
...
RobotDynamicsInterface::Ptr Controller::configureRobotDynamics(const ros::NodeHandle& nh)
{
    ...
    else if (_robot_type == "simple_car_delay")
    {
        double wheelbase = 0.5;
        double T_1 = 0.8;
        double T_2 = 0.8;
        nh.param("robot/simple_car_delay/wheelbase", wheelbase, wheelbase);
        nh.param("robot/simple_car_delay/time_constant/T_1", T_1, T_1);
        nh.param("robot/simple_car_delay/time_constant/T_2", T_2, T_2);
        return std::make_shared<SimpleCarDelayModel>(wheelbase, T_1, T_2);
    }
    else
    {
                                                                     ...
                                                                     const std::vector<teb_local_planner::PoseSE2>& via_points)
{
    ...
    {
        double max_vel_x = 0.4;
        nh.param("robot/simple_car_delay/max_vel_x", max_vel_x, max_vel_x);
        double max_vel_x_backwards = 0.2;
        nh.param("robot/simple_car_delay/max_vel_x_backwards", max_vel_x_backwards, max_vel_x_backwards);
        if (max_vel_x_backwards < 0)
        {
            ROS_WARN("max_vel_x_backwards must be >= 0");
            max_vel_x_backwards *= -1;
        ...
        double max_steering_angle = 1.5;
        nh.param("robot/simple_car_delay/max_steering_angle", max_steering_angle, max_steering_angle);
    ...
    {
        double acc_lim_x = 0.0;
        nh.param("robot/simple_car_delay/acc_lim_x", acc_lim_x, acc_lim_x);
        double dec_lim_x = 0.0;
        nh.param("robot/simple_car_delay/dec_lim_x", dec_lim_x, dec_lim_x);
        if (dec_lim_x < 0)
        {
            ROS_WARN("dec_lim_x must be >= 0");
            dec_lim_x *= -1;
        ...
        double max_steering_rate = 0.0;
        nh.param("robot/simple_car_delay/max_steering_rate", max_steering_rate, max_steering_rate);
...
bool Controller::isPoseTrajectoryFeasible(teb_local_planner::CostmapModel* costmap_model, const std::vector<geometry_msgs::Point>& footprint_spec,
