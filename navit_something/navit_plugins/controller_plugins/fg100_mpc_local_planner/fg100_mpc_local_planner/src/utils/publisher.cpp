...
#include <fg100_mpc_local_planner/utils/conversion.h>
...
void Publisher::publishLocalPlan(const std::vector<geometry_msgs::PoseStamped>& local_plan) const
{
    ...
    teb_local_planner::publishPlan(local_plan, _local_plan_pub);
}
...
{
    if (!_initialized) return;
    teb_local_planner::publishPlan(global_plan, _global_plan_pub);
}
