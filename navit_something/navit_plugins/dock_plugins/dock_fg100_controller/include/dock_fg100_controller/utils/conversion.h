 ...
 *
 * @remarks Note, the actual data of the TimeSeries object is copied without
 * interpolation.
 *
 * @todo We could avoid the system dynamics dependency by specifying a generic
 * getter function for the SE2 poses
 *
 * @param[in] time_series       corbo::TimeSeries object or any child class
 * @param[in] dynamics          Reference to the robot dynamics interface (to
 * access state-to-SE2 conversion methods)
 * @param[out] poses_stamped    The resulting pose array (note, the incoming
 * vector will be cleared)
 * @param[in] frame_id          The planning frame id that is added to the
 * message header
 */
void convert(const corbo::TimeSeries &time_series,
