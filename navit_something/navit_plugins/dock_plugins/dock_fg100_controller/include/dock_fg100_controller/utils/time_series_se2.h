 ...
 * Hereby, the third state component is always treated as orientation in [-pi,
 * pi).
 * In particular, whenever values are retrieved by interpolation
 * (getValuesInterpolate()), the third state is ensured to remain properly in
 * [-pi, pi).
 *
 * Note, as the first three state components define the SE2, any further
 * auxiliary
...
public:
  ...
  /**
   * @brief Retrieve value vector at a desired time stamp (seconds) via
   * interpolation
   *
   ...
   * @param[in]  interpolation     interpolation method according to
   * Interpolation enum
   * @param[in]  extrapolate       specify whether exrapliation should be
   * applied with a similar
   *                               method like interpolation
   * @param[in]  tolerance         specify a tolerance for which two time stamps
   * are treated equally
   * @returns true if successfull, false otherwise (e.g. if extrapolate is false
   * but \c time > max(time)
   */
  bool getValuesInterpolate(
      double time, Eigen::Ref<Eigen::VectorXd> values,
      Interpolation interpolation = Interpolation::Linear,
      Extrapolation extrapolate = Extrapolation::NoExtrapolation,
      double tolerance = 1e-6) const override;
   ...
   * @param[out]  mean_values     Resulting cwise mean values
   * [getValueDimension() x 1] (vector size must be preallocated)
   */
