...
public:
   ...
   *
   * @param[in]  x           Reference to the state vector [getStateDimension()
   * x 1]
   * @param[out] pos_x       X-component of the position
   * @param[out] pos_y       Y-component of the position
   */
  virtual void getPositionFromState(const Eigen::Ref<const Eigen::VectorXd> &x,
                                    double &pos_x, double &pos_y) const = 0;
   ...
   */
  virtual void getPoseSE2FromState(const Eigen::Ref<const Eigen::VectorXd> &x,
   ...
   * and hence it is assumed that a steady state for this particular pose is
   * avaiable,
   * which means that there exist a control u such that f(x,u) = 0 (continuous
   * time) or
   * f(x,u) = x (discrete time).
   *
   * For example, in case the remaining state components correspond to just
   * integrators,
   ...
   * @param[out] x           Reference to the state vector [getStateDimension()
   * x 1]
   */
  virtual void
  getSteadyStateFromPoseSE2(double pos_x, double pos_y, double theta,
                            Eigen::Ref<Eigen::VectorXd> x) const = 0;

  /**
   * @brief Convert PoseSE2 to steady state
   *
   * See description of getSteadyStateFromPoseSE2(double pos_x, double pos_y,
   * double theta, Eigen::Ref<Eigen::VectorXd> x)
   *
   * @param[in]  pose        PoseSE2 object
   * @param[out] x           Reference to the state vector [getStateDimension()
   * x 1]
   */
  virtual void getSteadyStateFromPoseSE2(const PoseSE2 &pose,
   ...
   *
   * Convert the control vector to a twist message (containing velocity
   * information)
   * if possible.
   *
   * @todo Maybe add current state x as optional input to allow for computing a
   * twist out of the first state and control
   *
   * @param[in]  u           Reference to the control vector
   * [getInputDimension() x 1]
   * @param[out] twist       Reference to the twist message
   * @return true, if conversion was successful, false otherwise
   */
  virtual bool getTwistFromControl(const Eigen::Ref<const Eigen::VectorXd> &u,
                                   geometry_msgs::Twist &twist) const = 0;

  /**
   * @brief Merge custom state feedback with pose and twist feedback
   *
   * Many robots in ROS publish their state information (pose, linear and
   * angular velocity)e.g. via an odometry message
   * (potentially corrected by localization).
   * However, for more complex robot models, it might not be possible to obtain
   * the full state information without any observer
   * or further measurements.
   * For this purpose, custom state feedback can be provided. But as the
   * complete navigation stack relies on the information obtained by
   * odometry, this method allows for merging of both sources.
   * In particular, it overrides only state components related to the pose and
   * velocity.
   *
   ...
   * @param[in,out] x           Custom state feedback in which related will be
   * overriden [getStateDimension() x 1]
   * @return true, if merging was successful, false otherwise
   */
  virtual bool
  mergeStateFeedbackAndOdomFeedback(const PoseSE2 &odom_pose,
