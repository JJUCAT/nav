...
/**
 * @brief Specialization of RobotDynamicsInterface for mobile robots operating
 * in SE2
 *
 * This abstract class defines a base class for robots in which the full state
 * vector x
 * is embedded in SE2, i.e. x = [pos_x, pos_y, theta] with pos_x, pos_y as real
 * numbers
 * and theta in [-pi, pi).
 *
 * Note, these models allow for simple conversion between state vectors and
 * poses.
 *
...
public:
  ...
  // implements interface method
  void getPositionFromState(const Eigen::Ref<const Eigen::VectorXd> &x,
                            double &pos_x, double &pos_y) const override {
  ...
  // implements interface method
  void getPoseSE2FromState(const Eigen::Ref<const Eigen::VectorXd> &x,
  ...
  // implements interface method
  void getSteadyStateFromPoseSE2(double pos_x, double pos_y, double theta,
    ...
    x[2] = theta;
    if (x.size() > 3)
      x.tail(x.size() - 3).setZero();
  }

  // implements interface method
  virtual bool mergeStateFeedbackAndOdomFeedback(
      const PoseSE2 &odom_pose, const geometry_msgs::Twist &odom_twist,
      Eigen::Ref<Eigen::VectorXd> x) const override {
    assert(x.size() == getStateDimension());
    x[0] = odom_pose.x();
    x[1] = odom_pose.y();
  ...
  void dynamics(const Eigen::Ref<const StateVector> &x,
                ...
                Eigen::Ref<StateVector> f) const override = 0;
};
}
