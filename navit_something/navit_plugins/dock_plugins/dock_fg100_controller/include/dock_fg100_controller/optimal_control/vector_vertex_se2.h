...
public:
  ...
  explicit VectorVertexSE2(int dimension, bool fixed = false)
      : VectorVertex(dimension, fixed) {}

  //! Construct vertex with given values
  explicit VectorVertexSE2(const Eigen::Ref<const Eigen::VectorXd> &values,
                           bool fixed = false)
      : VectorVertex(values, fixed) {}
  ...
  void plus(const double *inc) override {
    ...
    if (getDimension() > 3)
      _values.tail(getDimension() - 3) +=
          Eigen::Map<const Eigen::VectorXd>(inc + 3, getDimension() - 3);
  }
  // implements interface method
  ...
  virtual void set(const Eigen::Ref<const Eigen::VectorXd> &values,
                   const Eigen::Ref<const Eigen::VectorXd> &lb,
                   const Eigen::Ref<const Eigen::VectorXd> &ub,
                   bool fixed = false) {
...
public:
  ...
  explicit PartiallyFixedVectorVertexSE2(int dimension)
      : VectorVertexSE2(dimension),
        _fixed(Eigen::Array<bool, -1, 1>::Constant(dimension, false)),
        _num_unfixed(dimension) {}
  ...
  explicit PartiallyFixedVectorVertexSE2(
      ...
      : VectorVertexSE2(values),
        _fixed(Eigen::Array<bool, -1, 1>::Constant(values.size(), false)),
        _num_unfixed(values.size()) {}

  //! Construct vertex with given values and fixed components
  explicit PartiallyFixedVectorVertexSE2(
      const Eigen::Ref<const Eigen::VectorXd> &values,
      const Eigen::Ref<const Eigen::Array<bool, -1, 1>> &fixed)
      : VectorVertexSE2(values), _fixed(fixed),
        _num_unfixed(fixed.size() - fixed.count()) {}
  ...
  //! Set values and bounds at once
  void set(const Eigen::Ref<const Eigen::VectorXd> &values,
           const Eigen::Ref<const Eigen::VectorXd> &lb,
           const Eigen::Ref<const Eigen::VectorXd> &ub,
  ...
  bool hasFixedComponents() const override {
    return _num_unfixed < getDimension();
  }
  // implements interface method
  ...
  int getNumberFiniteLowerBounds(bool unfixed_only) const override {
    if (unfixed_only && _num_unfixed > 0) {
      int num = 0;
      for (int i = 0; i < getDimension(); ++i) {
        if (!_fixed[i] && _lb[i] > -corbo::CORBO_INF_DBL)
          num += 1;
      }
      return num;
    } else
  ...
  int getNumberFiniteUpperBounds(bool unfixed_only) const override {
    if (unfixed_only && _num_unfixed > 0) {
      int num = 0;
      for (int i = 0; i < getDimension(); ++i) {
        if (!_fixed[i] && _ub[i] < corbo::CORBO_INF_DBL)
          num += 1;
      }
      return num;
    } else
  ...
  int getNumberFiniteBounds(bool unfixed_only) const override {
    if (unfixed_only && _num_unfixed > 0) {
      ...
      for (int i = 0; i < getDimension(); ++i) {
        if (!_fixed[i] &&
            (_ub[i] < corbo::CORBO_INF_DBL || _lb[i] > -corbo::CORBO_INF_DBL))
          num += 1;
      }
      return num;
    } else
      return (_ub.array() < corbo::CORBO_INF_DBL ||
              _lb.array() > -corbo::CORBO_INF_DBL)
          .count();
  }
