 ...
 * \f[
 *    l(x, u) = (x_{ref} - x)^T Q (x_{xref} - x) + (u_{ref} - u)^T R (u_{ref} -
 * u)
 * \f]
 * However, this class ensures that the state distance (x_{ref} - x) is
 * computed properly in SO(2) for the third state component.
 *
 * Note, the class also provides a least squares form if not in integral form
 * which is:
 * \f$ l_{lsq} = \begin{bmatrix} \sqrt{Q} (x_{ref} - x) \\ \sqrt{R} (u_{ref} -
 * u) \end{bmatrix} \f$
 * \f$ \sqrt{Q} \f$  \f$ \sqrt{R} \f$ are computed once using the cholesky
 * decomposition.
...
public:
   ...
   *
   * @param[in] Q                Positive definite state weighting matrix (must
   * match dimensions!)
   * @param[in] R                Positive definite control weighting matrix
   * (must match dimensions!)
   * @param[in] integral_form    If true, the running costs are later integrated
   * over the grid interval,
   *                             otherwise, the values are just summed up as in
   * sampled-data MPC.
   * @param[in] lsq_form         Set to true in order to prefer the
   * least-squares form
   */
  QuadraticFormCostSE2(const Eigen::Ref<const Eigen::MatrixXd> &Q,
                       const Eigen::Ref<const Eigen::MatrixXd> &R,
                       bool integral_form = false, bool lsq_form = false)
      : corbo::QuadraticFormCost(Q, R, integral_form, lsq_form) {}

  // implements interface method
  corbo::StageCost::Ptr getInstance() const override {
    return std::make_shared<QuadraticFormCostSE2>();
  }

  // implements interface method
  void
  computeNonIntegralStateTerm(int k,
                              const Eigen::Ref<const Eigen::VectorXd> &x_k,
                              Eigen::Ref<Eigen::VectorXd> cost) const override;

  // implements interface method
  void computeIntegralStateControlTerm(
      int k, const Eigen::Ref<const Eigen::VectorXd> &x_k,
...
public:
   ...
   * least-squares form
   */
  QuadraticStateCostSE2(const Eigen::Ref<const Eigen::MatrixXd> &Q,
                        bool integral_form = false, bool lsq_form = false)
      : corbo::QuadraticStateCost(Q, integral_form, lsq_form) {}

  // implements interface method
  corbo::StageCost::Ptr getInstance() const override {
    return std::make_shared<QuadraticStateCostSE2>();
  }
