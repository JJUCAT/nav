...
public:
  ...
  QuadraticFinalStateCostSE2(const Eigen::Ref<const Eigen::MatrixXd> &Qf,
                             bool lsq_form = false)
      : corbo::QuadraticFinalStateCost(Qf, lsq_form) {}

  corbo::FinalStageCost::Ptr getInstance() const override {
    return std::make_shared<QuadraticFinalStateCostSE2>();
  }
...
public:
  ...
  TerminalBallSE2(const Eigen::Ref<const Eigen::MatrixXd> &S, double gamma)
      : corbo::TerminalBall(S, gamma) {}

  corbo::FinalStageConstraint::Ptr getInstance() const override {
    return std::make_shared<TerminalBallSE2>();
  }
