 ...
 *
 * Forward differences approximate \f$ \dot{x} = f(x, u) \f$ in the following
 * manner:
 * \f[
...
class ForwardDiffCollocationSE2
    : public corbo::FiniteDifferencesCollocationInterface {
public:
  // Implements interface method
  corbo::FiniteDifferencesCollocationInterface::Ptr
  getInstance() const override {
    return std::make_shared<ForwardDiffCollocationSE2>();
  }

  // Implements interface method
  void computeEqualityConstraint(const StateVector &x1, const InputVector &u1,
                                 const StateVector &x2, double dt,
 ...
 *
 * Midpoint differences approximate \f$ \dot{x} = f(x, u) \f$ in the following
 * manner:
 * \f[
...
public:
  ...
  corbo::FiniteDifferencesCollocationInterface::Ptr
  getInstance() const override {
    return std::make_shared<MidpointDiffCollocationSE2>();
  }
  ...
  void computeEqualityConstraint(const StateVector &x1, const InputVector &u1,
    ...
    midpoint.coeffRef(2) =
        interpolate_angle(x1.coeffRef(2), x2.coeffRef(2), 0.5);
 ...
 *
 * Crank-Nicolson differences approximate \f$ \dot{x} = f(x, u) \f$ in the
 * following manner:
 * \f[
...
public:
  ...
  corbo::FiniteDifferencesCollocationInterface::Ptr
  getInstance() const override {
    return std::make_shared<CrankNicolsonDiffCollocationSE2>();
  }
  ...
  void computeEqualityConstraint(const StateVector &x1, const InputVector &u1,
    ...
    error.head(2) -=
        (x2.head(2) - x1.head(2)) / dt - 0.5 * (f1.head(2) + error.head(2));
    error.coeffRef(2) -= normalize_theta(x2.coeffRef(2) - x1.coeffRef(2)) / dt -
                         0.5 * (f1.coeffRef(2) + error.coeffRef(2));
    if (x1.size() > 3) {
      int n = x1.size() - 3;
      error.tail(n) -=
          (x2.tail(n) - x1.tail(n)) / dt - 0.5 * (f1.tail(n) + error.tail(n));
    }
  }
};
}
