...
#include <dock_fg100_controller/optimal_control/vector_vertex_se2.h>
 ...
 *
 * Note the full discretization grid currently supports only finite difference
 * collocation
 * to discretize the continuous time dynamics.
 * Full discretization is also possible with multiple shooting and (expicit)
...
class FullDiscretizationGridBaseSE2
    : public corbo::DiscretizationGridInterface {
public:
  ...
  // implements interface method
  void getStateAndControlTimeSeries(
      TimeSeries::Ptr x_sequence, TimeSeries::Ptr u_sequence,
      double t_max = corbo::CORBO_INF_DBL) const override;
  ...
  void setFiniteDifferencesCollocationMethod(
  ...
  void setCostIntegrationRule(CostIntegrationRule integration) {
    _cost_integration = integration;
  }

  /**
   * @brief Find the closest pose (first part of the state vector) on the grid
   * w.r.t. to a provided reference point.
   *
   ...
   */
  int findClosestPose(double x_ref, double y_ref, int start_idx = 0,
...
protected:
  virtual void initializeSequences(const Eigen::VectorXd &x0,
  ...
  virtual bool adaptGrid(bool new_run, NlpFunctions &nlp_fun) {
    return false;
  } // A subclass might add a grid adaptation, returns true if adapted
  ...
  virtual void createEdges(NlpFunctions &nlp_fun, OptimizationEdgeSet &edges,
                           SystemDynamicsInterface::Ptr dynamics) = 0;
  ...
  corbo::FiniteDifferencesCollocationInterface::Ptr _fd_eval =
      std::make_shared<corbo::CrankNicolsonDiffCollocation>();
  ...
  int _n_ref = 11;
  int _n_adapt = 0; // if adaption is on and warmstart off, we might use this n
                    // instead of n_ref (only if n_adapt > 0)
  double _dt_ref = 0.1;
  ScalarVertex _dt; // we need a ScalarVertex to use the helper methods in
                    // stage_functions.cpp
