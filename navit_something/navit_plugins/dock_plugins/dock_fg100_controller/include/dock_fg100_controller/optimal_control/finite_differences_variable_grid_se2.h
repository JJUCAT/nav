 ...
 *
 * This class implements a full discretization grid with finite difference
 * collocation.
 * The temporal resolution is free.
 * This grid corresponds to the global uniform grid in time-optimal MPC as
 * defined in [1,2].
 *
 * [1] C. Rösmann, F. Hoffmann und T. Bertram: Timed-Elastic-Bands for
 * Time-Optimal Point-to-Point Nonlinear Model Predictive Control, ECC 2015.
 * [2] C. Rösmann: Time-optimal nonlinear model predictive control - Direct
 * transcription methods with variable discretization and structural sparsity
...
public:
  ...
  corbo::DiscretizationGridInterface::Ptr getInstance() const override {
    return std::make_shared<FiniteDifferencesVariableGridSE2>();
  }

  //! Get access to the associated factory
  static corbo::Factory<corbo::DiscretizationGridInterface> &getFactory() {
    return corbo::Factory<corbo::DiscretizationGridInterface>::instance();
  }
  ...
  void setGridAdaptTimeBasedAggressiveEstimate(int n_max,
