...
bool TimeSeriesSE2::getValuesInterpolate(double time,
  ...
  auto it = std::find_if(_time.begin(), _time.end(), [time](double val) {
    return val >= time;
  ...
  if (it == _time.end()) {
    ...
    default: {
      PRINT_ERROR("TimeSeries::valuesInterpolate(): desired extrapolation "
  ...
  if (idx >= getTimeDimension())
    idx = getTimeDimension() -
          1; // this might occure with floating point comparison
  ...
  // okay, so now we have the above case val > time -> so idx corresponds to the
  // next sample.
  double dt = time - _time[idx - 1];
  PRINT_ERROR_COND_NAMED(dt < 0,
                         "dt < 0 in interpolation. This cannot be correct.");
  ...
  case Interpolation::Linear: {
    if (idx < 1) {
      ...
      values.noalias() = getValuesMap(idx - 1) +
                         dt_frac * (getValuesMap(idx) - getValuesMap(idx - 1));
      // overwrite third component with is an angle in [-pi, pi)
      if (values.size() > 2) {
        values[2] = interpolate_angle(getValuesMap(idx - 1)[2],
                                      getValuesMap(idx)[2], dt_frac);
      }
    }
    break;
  }
  default: {
    PRINT_ERROR("TimeSeries::valuesInterpolate(): desired interpolation method "
                "not implemented.");
    return false;
  }
  }
...
void TimeSeriesSE2::computeMeanCwise(Eigen::Ref<Eigen::VectorXd> mean_values) {
  ...
  if (mean_values.size() != getValueDimension()) {
    PRINT_ERROR("TimeSeries::computeMeanCwise(): provided mean_values vector "
                "does not match value dimension");
    return;
  }
