...
corbo::GridUpdateResult FullDiscretizationGridBaseSE2::update(
    const Eigen::VectorXd &x0, ReferenceTrajectoryInterface &xref,
    ReferenceTrajectoryInterface &uref, NlpFunctions &nlp_fun,
    OptimizationEdgeSet &edges, SystemDynamicsInterface::Ptr dynamics,
    bool new_run, const corbo::Time &t, ReferenceTrajectoryInterface *sref,
    const Eigen::VectorXd *prev_u, double prev_u_dt,
  ...
  if (isGridAdaptActive() && !_first_run &&
      !isEmpty()) // TODO(roesmann): this might not be efficient in case warm
                  // start is deactivated
  {
    // adapt grid if implemented by subclass
    adaptGrid(new_run, nlp_fun); // note, isModified() should return true if
                                 // something was changed!, we could also use
                                 // the return value...
  }

  // check if we need to cache the reference trajectory values //
  // TODO(roesmann): we could restrict this to new_run==true only as long as we
  // do not
  // have a grid resize...
  int n = std::max(std::max(getNRef(), getN()), _n_adapt);
  if (_dt.value() <= 0)
    _dt.value() = _dt_ref; // initialize _dt to _dt_ref in case it has not been
                           // optimized before
  if (!xref.isCached(getDt(), n, t))
  ...
  if (prev_u && prev_u->size() > 0)
    ...
    setPreviousControl(Eigen::VectorXd::Zero(dynamics->getInputDimension()),
                       prev_u_dt);
  ...
  if (isEmpty()) // TODO(roesmann): threshold?: if (_cached_xf.rows() > 0 && (xf
                 // - _cached_xf).norm() > _warm_start_goal_dist_reinit)
  {
    if (xref.isStatic() && !xinit) {
      // TODO(roesmann): optional initialization references x_init, u_init
      initializeSequences(x0, xref.getReferenceCached(n - 1),
                          uinit ? *uinit : uref, nlp_fun);
    } else {
      initializeSequences(x0, xref.getReferenceCached(n - 1),
                          xinit ? *xinit : xref, uinit ? *uinit : uref,
                          nlp_fun);
    }
  } else {
    ...
    if (new_run) {
      // make sure to always overwrite the start with the actual (measured)
      // values
      _x_seq.front().values() = x0;
      // update fixed goal states
      for (int i = 0; i < _xf_fixed.size(); ++i) {
        if (_xf_fixed[i])
          _xf.values()[i] = xref.getReferenceCached(getN() - 1)[i];
      }
    }
  }

  result.vertices_updated = isModified(); // isModified() returns if the
                                          // underlying vertex set is updated

  if (new_run || result.updated()) // new run -> new t
  {
    // update NLP functions w.r.t. the current discretization
    result.edges_updated = nlp_fun.update(
        getN(), t.toSec(), xref, uref, sref, hasSingleDt(), x0, {getDt()},
        this); // returns true if edge dimensions changed
    // TODO(roesmann): sref is not yet implemented for testing, add this
    // later...
    // TODO(roesmann): we do not yet check if dt was updated, this might affect
    // nlp update
  }
  ...
  if (result.updated()) // vertices or eges updated
  {
    ...
    // TODO(roesmann): for now, we assume that we are the only maintainer of the
    // edge set (which might be not true generally!)
...
void FullDiscretizationGridBaseSE2::initializeSequences(
  ...
  // For the SE2 case here, we set the third state (orientation) to the goal
  // heading
  double orient_init = std::atan2(dir[1], dir[0]);
  // but check if goal is behind start pose (w.r.t. start orientation)
  if (dir.head(2).dot(Eigen::Vector2d(std::cos(x0[2]), std::sin(x0[2]))) < 0)
    orient_init = normalize_theta(orient_init + M_PI);

  for (int k = 0; k < num_intervals; ++k) {
    // add new state by linear interpolation
    Eigen::VectorXd new_x = x0 + (double)k * step * dir;
    if (k > 0)
      new_x[2] = orient_init; // but do not overwrite start orientation
...
void FullDiscretizationGridBaseSE2::initializeSequences(
  ...
  // if ((x0 - xref.getReferenceCached(0)).norm() >
  // _non_static_ref_dist_threshold)
  // {
  //    initializeSequences(x0, xref.getReferenceCached(getN() - 1), uref,
  //    nlp_fun);
  // }
  ...
  _x_seq.emplace_back(x0, nlp_fun.x_lb,
                      nlp_fun.x_ub); // always add the (exact) x0
...
void FullDiscretizationGridBaseSE2::warmStartShifting(
  ...
  // the simplest strategy would be to just remove remove x_seq[0], append xf to
  // x_seq and replace xf (num_shift=1)
  // however, this requries to change the structure which always triggers an
  // edge recreation and so the solver must reallocate memory.
  // TOOD(roesmann): for time-optimal stuff, if we are sure that we resize the
  // grid, it is then more efficent to implement the strategy above...
  ...
  for (int i = 0; i < num_shift; ++i, ++idx) {
    ...
    if (i == num_shift - 1) // consider xf
    {
      _xf.values() =
          _x_seq[idx - 2].values() +
          2.0 * (_x_seq[idx - 1].values() - _x_seq[idx - 2].values());
      // TODO(roesmann): we could avoid computing the third row in the previous
      // command, however, the code
      // increases if we support additional states
      _xf.values()[2] = interpolate_angle(_x_seq[idx - 2].values()[2],
                                          _x_seq[idx - 1].values()[2], 2.0);
    } else {
      ...
      // command, however, the code
      // increases if we support additional states
      _x_seq[idx].values()[2] = interpolate_angle(
          _x_seq[idx - 2].values()[2], _x_seq[idx - 1].values()[2], 2.0);
    }
...
int FullDiscretizationGridBaseSE2::findNearestState(const Eigen::VectorXd &x0) {
  ...
  if (std::abs(first_dist) < 1e-12)
    return 0;

  // find nearest state (using l2-norm) in order to prune the trajectory
  // (remove already passed states, should be 1 if the sampling times of the
  // planning and controller call match, but could be higher if rates
  // differ)
  ...
  double dist;
  int num_keep_interv = 1; // std::max(1, _num_states_min - 1);            // we
                           // need to keep at least this number of intervals in
                           // the trajectory
...
int FullDiscretizationGridBaseSE2::findClosestPose(double x_ref, double y_ref,
                                                   int start_idx,
  ...
  for (int i = start_idx; i < (int)_x_seq.size(); ++i) {
    double dist = distance_points2d(x_ref, y_ref, _x_seq[i].values()[0],
                                    _x_seq[i].values()[1]);
    if (dist < min_dist) {
      min_dist = dist;
      min_idx = i;
  ...
  double dist =
      distance_points2d(x_ref, y_ref, _xf.values()[0], _xf.values()[1]);
  if (dist < min_dist) {
    min_dist = dist;
...
bool FullDiscretizationGridBaseSE2::checkAndInitializeXfFixedFlags(int dim_x) {
  ...
  PRINT_ERROR_NAMED("Dimensions mismatch between xf_fixed and xf. Setting "
                    "xf_fixed to false.");
  _xf_fixed.setConstant(dim_x, false);
  return false;
}
...
void FullDiscretizationGridBaseSE2::updateBounds(const NlpFunctions &nlp_fun) {
  ...
  for (VectorVertex &vtx : _u_seq) {
    if (vtx.getDimension() == nlp_fun.u_lb.size())
      ...
      PRINT_ERROR_NAMED("Cannot update lower control input bounds due to "
                        "dimensions mismatch");

    if (vtx.getDimension() == nlp_fun.u_ub.size())
      vtx.setUpperBounds(nlp_fun.u_ub);
    else
      PRINT_ERROR_NAMED("Cannot update upper control input bounds due to "
                        "dimensions mismatch");
  }
...
void FullDiscretizationGridBaseSE2::resampleTrajectory(int n_new) {
  ...
  double t_new;
  int idx_old = 1;
  double t_old_p1 = dt_old; // time for old sample with index idx_old (acutally
                            // its the subsequent time step w.r.t t_new)

  for (int idx_new = 1; idx_new < n_new - 1;
       ++idx_new) // n_new-1 since last state is identical and copied later.
                  // idx_new=1, since start
  // sample is already valid (we do not touch it)
  // we allow a small mismatch for the control u_1 and let the optimizer correct
  // it later
  {
    t_new = dt_new * double(idx_new);
    while (t_new > double(idx_old) * dt_old && idx_old < n) {
      ++idx_old;
    }; // find idx_old that represents the state subsequent to the new one
       // (w.r.t. time)
    t_old_p1 = double(idx_old) * dt_old;

    const Eigen::VectorXd &x_prev = x_seq_old.col(idx_old - 1);
    const Eigen::VectorXd &x_cur =
        (idx_old < n - 1) ? x_seq_old.col(idx_old) : _xf.values();

    if (idx_new < num_interv) {
      // states
      _x_seq[idx_new].values() =
          x_prev + (t_new - (t_old_p1 - dt_old)) / dt_old * (x_cur - x_prev);
      // overwrite angle (since we are in SE2, at least the first three states
      // are SE2)
      // TODO(roesmann): we could avoid computing the third row in the previous
      // command, however, the code
      // increases if we support additional states
      _x_seq[idx_new].values()[2] = interpolate_angle(
          x_prev[2], x_cur[2], (t_new - (t_old_p1 - dt_old)) / dt_old);
      ...
      _x_seq.emplace_back(
          x_prev + (t_new - (t_old_p1 - dt_old)) / dt_old * (x_cur - x_prev),
          _nlp_fun->x_lb, _nlp_fun->x_ub);
      // TODO(roesmann): we could avoid computing the third row in the previous
      // command, however, the code
      // increases if we support additional states
      _x_seq.back().values()[2] = interpolate_angle(
          x_prev[2], x_cur[2], (t_new - (t_old_p1 - dt_old)) / dt_old);
      _u_seq.emplace_back(u_seq_old.col(idx_old - 1), _nlp_fun->u_lb,
                          _nlp_fun->u_ub);
    }
  }
  ...
  // _x_seq.front().setFixed(true); // should not be necessary here (we just
  // change values in the first node)
...
void FullDiscretizationGridBaseSE2::getVertices(
  ...
  vertices.push_back(
      &_dt); // make sure to make it fixed if desired in any subclass
...
void FullDiscretizationGridBaseSE2::computeActiveVertices() {
  ...
  for (int i = 0; i < n - 1; ++i) {
    if (!_x_seq[i].isFixed())
      _active_vertices.push_back(&_x_seq[i]);
    if (!_u_seq[i].isFixed())
      _active_vertices.push_back(&_u_seq[i]);
  }
  if (!_xf.isFixed())
    _active_vertices.push_back(&_xf);
  if (!_dt.isFixed())
    _active_vertices.push_back(&_dt);
}

void FullDiscretizationGridBaseSE2::getStateAndControlTimeSeries(
  ...
  if (u_sequence) {
    ...
    if (t <= t_max)
      u_sequence->add(t, _u_seq.back().values());
  }
}
}
