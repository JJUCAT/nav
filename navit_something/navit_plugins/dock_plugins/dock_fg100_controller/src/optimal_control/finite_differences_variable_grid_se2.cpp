...
void FiniteDifferencesVariableGridSE2::setGridAdaptTimeBasedSingleStep(
    int n_max, double dt_hyst_ratio, bool adapt_first_iter) {
...
void FiniteDifferencesVariableGridSE2::setGridAdaptTimeBasedAggressiveEstimate(
    int n_max, double dt_hyst_ratio, bool adapt_first_iter) {
...
void FiniteDifferencesVariableGridSE2::setGridAdaptSimpleShrinkingHorizon(
    bool adapt_first_iter) {
  _grid_adapt = GridAdaptStrategy::SimpleShrinkingHorizon;
  _adapt_first_iter = adapt_first_iter;
}

bool FiniteDifferencesVariableGridSE2::adaptGrid(bool new_run,
                                                 NlpFunctions &nlp_fun) {
  // do not adapt grid in a new run
  if (new_run && !_adapt_first_iter)
    return false;
...
bool FiniteDifferencesVariableGridSE2::adaptGridTimeBasedSingleStep(
    NlpFunctions &nlp_fun) {
  PRINT_WARNING_COND_NAMED(
      !isTimeVariableGrid(),
      "time based adaptation might only be used with a fixed dt.");
...
bool FiniteDifferencesVariableGridSE2::adaptGridTimeBasedAggressiveEstimate(
    NlpFunctions &nlp_fun) {
  PRINT_WARNING_COND_NAMED(
      !isTimeVariableGrid(),
      "time based adaptation might only be used with a fixed dt.");
  ...
  if (dt >= _dt_ref * (1.0 - _dt_hyst_ratio) &&
      dt <= _dt_ref * (1.0 + _dt_hyst_ratio))
    return false;

  // estimate number of samples based on the fraction dt/dt_ref.
  // dt is the time difference obtained in a previous solution (with a coarser
  // resp. finer trajectory resolution)
...
bool FiniteDifferencesVariableGridSE2::adaptGridSimpleShrinkingHorizon(
    NlpFunctions &nlp_fun) {
  int n = getN();
  if (n > _n_min) {
    resampleTrajectory(n - 1);
