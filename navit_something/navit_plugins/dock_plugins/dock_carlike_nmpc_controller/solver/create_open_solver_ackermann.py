...
doSim = True # Run simulation after creating the solver
...
Q = cs.DM.eye(NX) * [10.0, 10.0, 1.0]
R = cs.DM.eye(NU) * [1.0, 1.0]
QN = cs.DM.eye(NX) * [100.0, 100.0, 100.0]

# wheelbase
# FG100 is 2.0 m
L = 0.4 

# use simple car model
# assuming perfect model
# x_dot = u_0 * cos theta
# y_dot = u_0 * sin theta
# theta_dot = u_0/L * tan u_1
...
umin = [-0.15, -1.2] * N  # - cs.DM.ones(NU * N) * cs.inf
umax = [0.15, 1.2] * N  # cs.DM.ones(NU * N) * cs.inf
...
if doSim:
    ...
    u_sim = np.zeros((NSim + 1, NU))
    u_sim[0:N + 1] = current_u
