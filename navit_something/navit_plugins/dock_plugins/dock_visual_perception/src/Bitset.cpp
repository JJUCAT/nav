#include "dock_visual_perception/Bitset.h"
...
void BitsetExt::hamming_enc_block(unsigned long block_len,
                                  deque<bool>::iterator& iter)
{
  if (verbose)
  ...
  // Note, that the last parity bit can safely be removed from the code if it is
  // not desired...
...
int BitsetExt::hamming_dec_block(unsigned long block_len,
                                 deque<bool>::iterator& iter)
{
  if (verbose)
  ...
  if (block_len == (next_parity >> 1))
  {
    parity = parity & ~(next_parity >> 1);  // The last parity bit shouldn't be
                                            // included in the other parity
                                            // tests (TODO: Better solution)
    if (total_parity == 0)
    {
...
void BitsetExt::str_8to6bit() {
  // TODO: Assume that the bitset contains 8-bit ASCII chars and change them
into 6-bit chars
}
void BitsetExt::str_6to8bit() {
  // TODO: Assume that the bitset contains 6-bit chars and change them into
8-bit ASCII chars
}
