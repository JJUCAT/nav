...
inline gm::Quaternion makeQuaternion(double x, double y, double z, double w)
{
  gm::Quaternion q;
  q.x = x;
  q.y = y;
              ...
              double* c, double* d)
{
  ...
  const double s = coeffs.values[0] * coeffs.values[0] +
                   coeffs.values[1] * coeffs.values[1] +
                   coeffs.values[2] * coeffs.values[2];
  if (fabs(s) < 1e-6)
    return -1;
  *a = coeffs.values[0] / s;
  *b = coeffs.values[1] / s;
...
int getQuaternion(const tf::Matrix3x3& m, tf::Quaternion& retQ)
{
  ...
  // Use Eigen for this part instead, because the ROS version of bullet appears
  // to have a bug
