...
{
/**
 * \brief Templateless version of MarkerDetector. Please use MarkerDetector
 * instead.
 */
class AR_EXPORT MarkerDetectorBase
{
protected:
  virtual Marker* new_M(double _edge_length = 0, int _res = 0,
                        double _margin = 0) = 0;
...
public:
   ...
   * using (e.g. cm) \param _res The marker content resolution in pixels. By
   * default we use 5x5 markers. If you use 0 with \e MarkerData, the marker
   * resolution is detected automatically. \param _margin The marker margin
   * resolution in pixels (The actual captured marker image has pixel resolution
   * of _margin+_res+_margin)
   *
   * \note The default marker content resolution (_res) of 5 can only detect
   * marker ids from 0 to 255. For larger marker ids, you need to increase the
   * marker content resolution accordingly.
   */
  void SetMarkerSize(double _edge_length = 1, int _res = 5, double _margin = 2);

  /** Set marker size for specified marker id. This needs to be called after
   * setting the default marker size. \param id The specified marker id \param
   * _edge_length Length of the marker's edge in whatever units you are using
   * (e.g. cm)
   */
  void SetMarkerSizeForId(unsigned long id, double _edge_length = 1);

  /** Set marker size for specified marker id. This needs to be called after
   * setting the default marker size. \param _detect_pose_grayscale Do we detect
   * marker pose using grayscale optimization?
   */
   ...
   * - The corresponding marker corners and marker points are in marker
   * coordinates (x is to east, y is to north, and z is up from the marker)
   ...
   */
  int Detect(cv::Mat& image, Camera* cam, bool track = false,
             bool visualize = false, double max_new_marker_error = 0.08,
             double max_track_error = 0.2,
             LabelingMethod labeling_method = CVSEQ, bool update_pose = true);

  int DetectAdditional(cv::Mat& image, Camera* cam, bool visualize = false,
                       double max_track_error = 0.2);
};
 ...
 */
template <class M>
class AR_EXPORT MarkerDetector : public MarkerDetectorBase
{
protected:
...
public:
  ...
  /** Constructor */
  MarkerDetector() : MarkerDetectorBase()
  {
