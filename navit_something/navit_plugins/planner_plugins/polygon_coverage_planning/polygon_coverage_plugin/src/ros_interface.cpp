...
bool polygonFromMsg(const geometry_msgs::Polygon& outer, const std::vector<geometry_msgs::Polygon>& inner,
                    PolygonWithHoles* polygon){

  Polygon_2 hull;
  polygon2FromPolygonMsg(outer, &hull);
  if (hull.is_clockwise_oriented()) hull.reverse_orientation();

  std::list<Polygon_2> holes(inner.size());
  for (size_t i = 0; i < inner.size(); ++i) {
    auto hole = std::next(holes.begin(), i);
    polygon2FromPolygonMsg(inner[i], &(*hole));
    if (hole->is_counterclockwise_oriented()) hole->reverse_orientation();
  }
