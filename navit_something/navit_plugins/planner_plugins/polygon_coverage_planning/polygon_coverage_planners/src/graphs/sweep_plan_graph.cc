...
bool SweepPlanGraph::create() {
  ...
  ROS_INFO_STREAM("decomposition polygon clusters " << polygon_clusters_.size());
  ...
  for (size_t cluster = 0; cluster < polygon_clusters_.size(); ++cluster) {
    ROS_INFO_STREAM("cluster: " << cluster << "  , size:"<<polygon_clusters_[cluster].size());
    ...
    if (settings_.sweep_single_direction) {
      ...
      if (!fields2CoverComputeSweep(settings_.turn_type, settings_.route_type, settings_.turn_radius,
                                  settings_.robot_width, settings_.sensor_model->getSweepDistance(), polygon_clusters_[cluster], best_dir,
                                  settings_.shrink_factor,
      ...
      ROS_ASSERT(settings_.sensor_model);
      if (!fields2CoverComputeAllSweeps(
              settings_.turn_type, settings_.route_type, settings_.turn_radius,
              settings_.robot_width, settings_.sensor_model->getSweepDistance(),
              settings_.shrink_factor,
              polygon_clusters_[cluster], &cluster_sweeps)) {
    ...
    for (size_t i = 0; i < node_properties.size(); ++i) {
      NodeProperty node;
      if (!createNodeProperty(valid_node_, &cluster_sweeps[i], &node)) {
        return false;
      }
...
bool SweepPlanGraph::computeDecomposition() {
  ...
  std::vector<Polygon_2> polygon_decomposition;
    ...
    case DecompositionType::kBCD: {
      if (!computeBestBCDFromPolygonWithHoles(settings_.polygon,
        ...
        ROS_INFO_STREAM("Successfully created boustrophedon decomposition with "
                        << polygon_decomposition.size() << " polygon(s).");
      }
      break;
    }
  ...
  // todo merge polygon
  ROS_INFO_STREAM("--------------------------------todo merge polygon");
  std::vector<int> free(polygon_decomposition.size(), 1);
  free[0] = 0;
  polygon_clusters_.push_back(polygon_decomposition.front());
  bool get_last{true};

  while (true) {
    if (std::accumulate(free.begin(), free.end(), 0) == 0) {
      break;
    }
    if(!get_last){
      for (size_t i = 0; i < polygon_decomposition.size(); i++) {
        if (free[i] == 0) {
          continue;
        }
        polygon_clusters_.push_back(polygon_decomposition[i]);
        free[i] = 0;
        get_last = true;
        break;
      }
    }
    auto p1 = polygon_clusters_.back();
    for (size_t i = 0; i < polygon_decomposition.size(); i++) {
      get_last = false;
      if (free[i] == 0) {
        continue;
      }
      auto p2 = polygon_decomposition[i];

      if (!segment_intersect(p1, p2)) {
        continue;
      }

      Polygon_2 bound;
      if (!meet_merge(p1, p2, bound)) {
        continue;
      }

      polygon_clusters_.back() = bound;
      free[i] = 0;
      get_last = true;
      break;
    }
...
bool SweepPlanGraph::meet_merge(const Polygon_2& p1, const Polygon_2& p2,
                                Polygon_2& bound) {
  // 1. max area => min_altitude
  std::vector<Polygon_2> list_p{p1, p2};
  auto area_p1 = computeArea(p1);
  auto area_p2 = computeArea(p2);

  int index_best = area_p1 > area_p2 ? 0 : 1;
  int index_altitide = area_p1 > area_p2 ? 1 : 0;

  Direction_2 dir_best, m;
  auto altitude_best = findBestSweepDir(list_p[index_best], &dir_best);
  auto altitude_increase = findBestSweepDir(list_p[index_altitide], &m);

  PolygonWithHoles unionR;
  Polygon_2 unionP;
  if (CGAL::join(p1, p2, unionR)) {
    unionP = unionR.outer_boundary();
    simplifyPolygon(&unionP);
  }

  Direction_2 dir_union;
  auto altitude_union = findBestSweepDir(unionP, &dir_union);

  auto collinear = CGAL::orientation(dir_best.vector(), dir_union.vector()) == CGAL::COLLINEAR;
  auto increase_least = altitude_union - (altitude_best + altitude_increase);

  bound = unionP;
  auto merge = collinear && increase_least < 1e-5;

  // ROS_WARN(
  //     "merge:%s, collinear:%s, altitude_union:%lf, altitude_best + "
  //     "altitude_increase:%lf",
  //     merge ? "true" : "false", collinear ? "true" : "false", altitude_union,
  //     altitude_best + altitude_increase);
...
bool SweepPlanGraph::solve(const Point_2& start, const Point_2& goal,
  ...
  if (!createNodeProperty(valid_node_ + 1, start, &start_node) ||
      !createNodeProperty(valid_node_ + 2, goal, &goal_node)) {
    return false;
  }
...
bool SweepPlanGraph::solve(const Point_2& start, const Point_2& goal,
                           std::vector<std::vector<Point_2>>* waypoints) const {
  ROS_ASSERT(waypoints);
  waypoints->clear();
  ...
  std::vector<std::vector<int>> m = temp_gtsp_graph.getAdjacencyMatrix();
  if (m.size() == 0) {
    ROS_ERROR("Fcpp: getAdjacencyMatrix returned empty matrix.");
    return 0;
  }
...
bool SweepPlanGraph::getWaypoints(const Solution& solution,
  ...
  for (size_t i = 0; i < solution.size() - 1; ++i) {
    ...
    waypoints->push_back(node_property->waypoints);
