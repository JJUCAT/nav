...
class SweepPlanGraph : public GraphBase<NodeProperty, EdgeProperty> {
 public:
  struct Settings {
    ...
    bool offset_polygons = false;  // Flag to offset neighboring cells.
    bool sweep_single_direction = true;  // Flag to sweep only in best direction.
    int turn_type = 0; // 0: Boustrophedon, 1: Snake, 2: Spiral(6), 3: Spiral(4)
    int route_type = 0; // 0: DubinsCurves, 1: DubinsCurvesCC, 2: ReedsSheppCurves, 3: ReedsSheppCurvesHC
    double turn_radius = 0.4;
    double robot_width = 1.16;
    double use_start_end_point = false;
 ...
 private:
  ...
  bool meet_merge(const Polygon_2 &p1,const Polygon_2 &p2, Polygon_2 &bound);
