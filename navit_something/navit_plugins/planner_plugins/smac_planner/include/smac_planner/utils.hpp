...
inline geometry_msgs::Pose getWorldCoords(
  const float & mx, const float & my, const navit_costmap::Costmap2D * costmap)
{
...
*/
inline double findCircumscribedCost(std::shared_ptr<navit_costmap::Costmap2DROS> costmap)
{
  double result = -1.0;
  bool inflation_layer_found = false;
  std::vector<boost::shared_ptr<navit_costmap::Layer>>::iterator layer;
  ...
  for (layer = costmap->getLayeredCostmap()->getPlugins()->begin();
    ...
    ++layer)
  {
    boost::shared_ptr<navit_costmap::InflationLayer> inflation_layer =
      boost::dynamic_pointer_cast<navit_costmap::InflationLayer>(*layer);
    if (!inflation_layer) {
      continue;
    }
  ...
  if (!inflation_layer_found) {
    ROS_WARN(
      //ROS_INFO("computeCircumscribedCost"),
