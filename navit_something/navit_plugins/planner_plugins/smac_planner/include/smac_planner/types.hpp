...
struct SmootherParams
{
  ...
  void get(const std::string & name)
  {
    ...
    // Smoother params
    ros::NodeHandle pnh("~/" + local_name);
    pnh.param("tolerance", tolerance_, 1e-10);
    pnh.param("max_iterations", max_its_, 1000);
    pnh.param("w_data", w_data_, 0.2);
    pnh.param("w_smooth", w_smooth_, 0.3);
    pnh.param("do_refinement", do_refinement_, true);
    pnh.param("refinement_num", refinement_num_, 2);
