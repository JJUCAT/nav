...
public:
   ...
   */
  void initialize(
    //const navit_util::LifecycleNode::WeakPtr & node,
    ros::NodeHandle* node,
    const std::string & global_frame,
    const std::string & topic_name,
    navit_costmap::Costmap2D * const costmap,
   ...
   */
  navit_costmap::Costmap2D * downsample(const unsigned int & downsampling_factor);
...
protected:
  ...
  std::unique_ptr<navit_costmap::Costmap2D> _downsampled_costmap;
  std::unique_ptr<navit_costmap::Costmap2DPublisher> _downsampled_costmap_pub;
};
