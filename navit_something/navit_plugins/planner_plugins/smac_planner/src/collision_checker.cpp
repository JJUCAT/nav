...
void GridCollisionChecker::setFootprint(
  const navit_costmap::Footprint & footprint,
  const bool & radius,
  const double & possible_inscribed_cost)
{
  ...
  for (unsigned int i = 0; i != angles_.size(); i++) {
    ...
    navit_costmap::Footprint oriented_footprint;
  ...
  const bool & traverse_unknown)
{
  ...
  if (!footprint_is_radius_) {
    ...
    //     ROS_ERROR_THROTTLE(
    //       //logger_, *clock_,1000,
    //       1000, "Inflation layer either not found or inflation is not set sufficiently for "
    ...
    geometry_msgs::Point new_pt;
    const navit_costmap::Footprint & oriented_footprint = oriented_footprints_[angle_bin];
    navit_costmap::Footprint current_footprint;
