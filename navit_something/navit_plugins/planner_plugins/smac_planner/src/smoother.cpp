...
{
//using namespace navit_util::geometry_utils;  // NOLINT
...
bool Smoother::smooth(
  ...
  const navit_costmap::Costmap2D * costmap,
  const double & max_time)
{
  ...
  //geometry_msgs::PoseStamped curr_path_segment;
  ...
  for (unsigned int i = 0; i != path_segments.size(); i++) {
    if (path_segments[i].end - path_segments[i].start > 10) {
      ...
      ros::Duration time_span = now - start;
      time_remaining = max_time - time_span.toSec();
      refinement_ctr_ = 0;
  ...
  const double & max_time)
{
  ...
  while (change >= tolerance_) {
    ...
    ros::Duration timespan(ros::Duration(b - a));
    if (timespan > max_dur) {
      ROS_DEBUG(
        //rclcpp::get_logger("SmacPlannerSmoother"),
  ...
  const bool & reversing_segment)
{
  ...
  for (unsigned int i = 0; i != best_expansion.pts.size(); i++) {
    ...
    path.poses[expansion_starting_idx + i].pose.orientation = orientationAroundZAxis(best_expansion.pts[i].theta);
  }
}
