...
//#include "navit_util/geometry_utils.hpp"
  ...
  const std::shared_ptr<navit_costmap::Costmap2DROS>& costmap_ros)
{
  ...
  //ROS_INFO(_logger, "Configuring %s of type SmacPlanner2D", name.c_str());
  ...
  pnh.param("use_final_approach_orientation", _use_final_approach_orientation, false);
  pnh.param("max_planning_time", _max_planning_time, 2.0);
  ...
  const geometry_msgs::PoseStamped & goal)
{
  ...
  if (mx_start == mx_goal && my_start == my_goal) {
    if (costmap->getCost(mx_start, my_start) == navit_costmap::LETHAL_OBSTACLE) {
      //throw nav2_core::StartOccupied("Start was in lethal cost");
      error = std::string("Start was in lethal cost");
    }
...
PLUGINLIB_EXPORT_CLASS(nav2_smac_planner::SmacPlanner2D, navit_core::GlobalPlanner)