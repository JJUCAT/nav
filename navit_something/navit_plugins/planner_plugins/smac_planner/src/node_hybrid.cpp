...
navit_costmap::Costmap2D * NodeHybrid::sampled_costmap = nullptr;
...
void NodeHybrid::resetObstacleHeuristic(
  navit_costmap::Costmap2D * costmap,
  ...
  const unsigned int & goal_x, const unsigned int & goal_y)
{
  ...
  if (motion_table.downsample_obstacle_heuristic) {
    //std::weak_ptr<navit_utils::LifecycleNode> ptr;
    ros::NodeHandle ptr;
    downsampler.initialize(& ptr,"fake_frame", "fake_topic", costmap, 2.0, true);
