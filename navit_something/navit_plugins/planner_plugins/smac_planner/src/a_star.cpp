  ...
  std::vector<std::tuple<float, float, float>> * expansions_log)
{
  ...
  const unsigned long int max_index = (unsigned long int)getSizeX() * (unsigned long int)getSizeY() * (unsigned long int)getSizeDim3();

  NodeGetter neighborGetter =
    [&, this](const unsigned long int & index, NodePtr & neighbor_rtn) -> bool
    {
      if (index >= max_index) {
        return false;
      }
  ...
  while (iterations < getMaxIterations() && !_queue.empty()) {
    ...
    if (iterations % _timing_interval == 0) {
      ...
      {
         throw std::runtime_error("planner time exceed max_planning_time, stopping search");
      }
    }
...
void AStarAlgorithm<Node2D>::clearStart()
{
  ...
  _costmap->setCost(coords.x, coords.y, navit_costmap::FREE_SPACE);
}
