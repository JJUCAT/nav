...
// Function that creates a room exploration path for the given map, by using the morse cellular decomposition method
// proposed in
//
...
// I.	Using the Sobel operator the direction of the gradient at each pixel is computed. Using this information, the
// direction is
//		found that suits best for calculating the cells, i.e. such that longer cells occur, and the map is rotated in this
// manner. 		This allows to use the algorithm as it was and in the last step, the found path points simply will be
...
// III.	After all cells have been determined by the sweeping slice, the algorithm finds these by using
// cv::findContours().
...
//		the cell, what ensures that the whole area of the cell is covered. For each cell the longest edge is found and it
// is transformed 		s.t. this edge lies horizontal to the x-direction. This produces longer but fewer edges, what
// improves the path for small but long 		cells. The startpoint of the cell-path is determined by the endpoint of the
// previous cell, s.t. the distance between two 		cell-paths is minimized. The cell-path is determined in the rotated
// manner, so in a last step, the cell-path is transformed to 		the originally transformed cell and after that
// inserted in the global path.
// V.	The previous step produces a path for the field of view. If wanted this path gets mapped to the robot path s.t.
//		the field of view follows the wanted path. To do so simply a vector transformation is applied. If the computed
// robot 		pose is not in the free space, another accessible point is generated by finding it on the radius around the
// field of view (fov) 		middlepoint s.t. the distance to the last robot position is minimized. If this is not wanted
// one has to set the 		corresponding Boolean to false (shows that the path planning should be done for the robot
// footprint).
                                               ...
                                               const double min_cell_area, const int max_deviation_from_track)
{
  ...
  computeCellDecompositionWithRotation(room_map, map_resolution, min_cell_area, min_cell_width, 0., R, bbox,
                                       rotated_room_map, cell_polygons, polygon_centers);
  // does not work so well: findBestCellDecomposition(room_map, map_resolution, min_cell_area, R, bbox,
  // rotated_room_map, cell_polygons, polygon_centers);
  ...
  const cv::Point rotated_starting_point =
      starting_point_vector[0];  // point that keeps track of the last point after the boustrophedon path in each cell
  ...
  if (cell_visiting_order == OPTIMAL_TSP)
  {
    ...
    //	std::vector<int> optimal_order = tsp_solver.solveConcordeTSP(rotated_room_map, polygon_centers, 0.25, 0.0,
    // map_resolution, start_cell_index, 0);
    GeneticTSPSolver tsp_solver;
    optimal_order =
        tsp_solver.solveGeneticTSP(rotated_room_map, polygon_centers, 0.25, 0.0, map_resolution, start_cell_index, 0);
    if (optimal_order.size() != polygon_centers.size())
    {
      std::cout << "=====================> Genetic TSP failed with 25% resolution, falling back to 100%. "
                   "<======================="
                << std::endl;
      optimal_order =
          tsp_solver.solveGeneticTSP(rotated_room_map, polygon_centers, 1.0, 0.0, map_resolution, start_cell_index, 0);
    }
  }
  else if (cell_visiting_order == LEFT_TO_RIGHT)
  {
    // we are using an alternative ordering here, which visits the cells in a more obvious fashion to the human observer
  ...
  {
    std::cout << "Error: BoustrophedonExplorer::getExplorationPath: The specified cell_visiting_order="
              << cell_visiting_order << " is invalid." << std::endl;
    return;
  }
  ...
  cv::Point robot_pos =
      rotated_starting_point;  // point that keeps track of the last point after the boustrophedon path in each cell
  std::vector<cv::Point2f> fov_middlepoint_path;  // this is the trajectory of centers of the robot footprint or the
                                                  // field of view
  ...
  std::vector<geometry_msgs::Pose2D> fov_poses;  // this is the trajectory of poses of the robot footprint or the field
                                                 // of view, in [pixels]
  ...
  cv::circle(room_map_path, cv::Point(cvRound(fov_poses.back().x), cvRound(fov_poses.back().y)), 1, cv::Scalar(200),
             CV_FILLED);
  //	for(size_t i=0; i<optimal_order.size()-1; ++i)
  //		cv::line(room_map_path, polygon_centers[optimal_order[i]], polygon_centers[optimal_order[i+1]], cv::Scalar(100),
  // 1);
...
void BoustrophedonExplorer::findBestCellDecomposition(const cv::Mat& room_map, const float map_resolution,
                                                      const double min_cell_area, const int min_cell_width, cv::Mat& R,
                                                      ...
                                                      std::vector<cv::Point>& polygon_centers)
{
  ...
  computeCellDecompositionWithRotation(room_map, map_resolution, min_cell_area, min_cell_width, 0., R_1, bbox_1,
                                       rotated_room_map_1, cell_polygons_1, polygon_centers_1);
  computeCellDecompositionWithRotation(room_map, map_resolution, min_cell_area, min_cell_width, 90. / 180. * CV_PI, R_2,
                                       bbox_2, rotated_room_map_2, cell_polygons_2, polygon_centers_2);
...
void BoustrophedonExplorer::computeCellDecompositionWithRotation(const cv::Mat& room_map, const float map_resolution,
                                                                 ...
                                                                 const double rotation_offset, cv::Mat& R,
                                                                 cv::Rect& bbox, cv::Mat& rotated_room_map,
                                                                 std::vector<GeneralizedPolygon>& cell_polygons,
                                                                 std::vector<cv::Point>& polygon_centers)
{
...
//	cv::waitKey();
  ...
  // ***********************
  // *********************** III. Find the separated cells. ***********************
  computeCellDecomposition(rotated_room_map, map_resolution, min_cell_area, min_cell_width, cell_polygons,
                           polygon_centers);
}
                                                     ...
                                                     std::vector<cv::Point>& polygon_centers)
{
  ...
  for (size_t y = y_start + 1; y < room_map.rows;
       ++y)                      // start at y_start+1 because we know number of segments at y_start
  {
    ...
    bool obstacle_hit =
        false;  // bool to check if the line currently hit an obstacle, s.t. not all black pixels trigger an event
    bool hit_white_pixel = false;  // bool to check if a white pixel has been hit at the current slice, to start the
                                   // slice at the first white pixel
    ...
    // if the number of segments did not change, check whether the position of segments has changed so that there is a
    // gap between them
    ...
    else if (previous_number_of_segments > number_of_segments)  // OUT event
    {
      ...
      for (int x = 0; x < room_map.cols; ++x)
      {
        ...
        else if (hit_white_pixel == true && room_map.at<uchar>(y - 1, x) == 0)
        {
          ...
          bool critical_point = true;
          for (int dx = -1; dx <= 1; ++dx)
            if (room_map.at<uchar>(y, std::max(0, std::min(x + dx, room_map.cols - 1))) == 0)  // check at side after
                                                                                               // obstacle
              critical_point = false;
...
//	}
  ...
  for (size_t cell = 0; cell < cells.size(); ++cell)
  {
    ...
    {
      std::cout << "WARN: BoustrophedonExplorer::computeCellDecomposition: dropped cell " << cell + 1
                << " with area=" << current_cell.getArea() << ". This should only happen for small unconnected cells."
                << std::endl;
    }
  }
}
                                      ...
                                      const int min_cell_width)
{
  ...
  std::map<int, boost::shared_ptr<BoustrophedonCell> > cell_index_mapping;  // maps each cell label --> to the cell
                                                                            // object
  int label_index = 1;
  for (int v = 0; v < cell_map_labels.rows; ++v)
  {
    for (int u = 0; u < cell_map_labels.cols; ++u)
    {
      ...
      cell_index_mapping[label_index] =
          boost::shared_ptr<BoustrophedonCell>(new BoustrophedonCell(label_index, area, bounding_box));
      label_index++;
      if (label_index == INT_MAX)
        std::cout << "WARN: BoustrophedonExplorer::mergeCells: label_index exceeds range of int." << std::endl;
    }
  }
  std::cout << "INFO: BoustrophedonExplorer::mergeCells: found " << label_index - 1 << " cells before merging."
            << std::endl;
...
//	cv::waitKey();
  ...
  std::cout << "INFO: BoustrophedonExplorer::mergeCells: " << cell_index_mapping.size()
            << " cells remaining after merging." << std::endl;
  return cell_index_mapping.size();
}

void BoustrophedonExplorer::mergeCellsSelection(
    cv::Mat& cell_map, cv::Mat& cell_map_labels,
    std::map<int, boost::shared_ptr<BoustrophedonCell> >& cell_index_mapping, const double min_cell_area,
    const int min_cell_width)
{
  // iteratively merge cells
  // merge small cells below min_cell_area with their largest neighboring cell
  std::multimap<double, boost::shared_ptr<BoustrophedonCell> > area_to_region_id_mapping;  // maps the area of each cell
  ...
  for (int v = 1; v < cell_map.rows - 1; ++v)
  {
    for (int u = 1; u < cell_map.cols - 1; ++u)
    {
      if (cell_map.at<uchar>(v, u) == BORDER_PIXEL_VALUE)
      {
        ...
        {
          int new_label = -1;
          for (std::multimap<double, boost::shared_ptr<BoustrophedonCell> >::reverse_iterator it =
                   area_to_region_id_mapping.rbegin();
                                          ...
                                          std::map<int, boost::shared_ptr<BoustrophedonCell> >& cell_index_mapping)
{
  ...
  for (int v = 0; v < cell_map.rows; ++v)
    for (int u = 0; u < cell_map.cols; ++u)
      if (cell_map.at<uchar>(v, u) == BORDER_PIXEL_VALUE && ((cell_map_labels.at<int>(v, u - 1) == minor_cell.label_ &&
...
void BoustrophedonExplorer::correctThinWalls(cv::Mat& room_map)
{
  for (int v = 1; v < room_map.rows; ++v)
  {
    ...
    {
      if (room_map.at<uchar>(v - 1, u - 1) == 255 && room_map.at<uchar>(v - 1, u) == 0 &&
          room_map.at<uchar>(v, u - 1) == 0 && room_map.at<uchar>(v, u) == 255)
        room_map.at<uchar>(v, u) = 0;
      else if (room_map.at<uchar>(v - 1, u - 1) == 0 && room_map.at<uchar>(v - 1, u) == 255 &&
               room_map.at<uchar>(v, u - 1) == 255 && room_map.at<uchar>(v, u) == 0)
...
void BoustrophedonExplorer::computeBoustrophedonPath(const cv::Mat& room_map, const float map_resolution,
                                                     ...
                                                     const int half_grid_spacing_as_int, const double path_eps,
                                                     const int max_deviation_from_track, const int grid_obstacle_offset)
{
  // get a map that has only the current cell drawn in
  //	Remark:	single cells are obstacle free so it is sufficient to use the cell to check if a position can be reached
  // during the 			execution of the coverage path
  cv::Mat cell_map;
  ...
  // this was deactivated because it is not as accurate as the direct check within
  // GridGenerator::generateBoustrophedonGrid, because the rotation introduces some rounding errors
  ...
  // upper and lower horizontal path (possible nearest locations) for the edges transformed to the original coordinates
  // (easier)
  ...
  cv::Mat R_cell_inv;
  cv::invertAffineTransform(R_cell, R_cell_inv);  // invert the rotation matrix to remap the determined points to the
                                                  // original cell
  ...
  bool start_from_left = (min_corner_index % 2 == 0 ? true : false);  // boolean to determine on which side the path
                                                                      // should start and to check where the path ended
  ...
  if (start_from_upper_path == true)  // plan the path starting from upper horizontal line
  {
    ...
    {
      if (start == true)  // at the beginning of path planning start at first horizontal line --> no vertical points
                          // between lines
      {
        if (start_from_left == true)
      ...
      if (start_from_left == true)  // plan path from left to right corner
      {
        ...
        path_planner_.planPath(rotated_inflated_cell_map, cell_robot_pos, line->upper_line[0], 1.0, 0.0, map_resolution,
                               0, &astar_path);
  ...
  // 从上一个区域的最后一个点到本区域的起点。
  std::vector<cv::Point> astar_path_goto_cell_start;
  std::vector<cv::Point> current_fov_path_goto_cell_start;
  path_planner_.planPath(room_map, robot_pos, outer_corners[min_corner_index], 1.0, 0.0, map_resolution, 0,
                         &astar_path_goto_cell_start);
  downsamplePath(astar_path_goto_cell_start, current_fov_path_goto_cell_start, robot_pos, path_eps);
  std::vector<cv::Point2f> fov_middlepoint_path_part_goto_cell_start;
  for (std::vector<cv::Point>::iterator point = current_fov_path_goto_cell_start.begin();
       point != current_fov_path_goto_cell_start.end(); ++point)
    fov_middlepoint_path_part_goto_cell_start.push_back(cv::Point2f(point->x, point->y));

  fov_middlepoint_path.insert(fov_middlepoint_path.end(), fov_middlepoint_path_part_goto_cell_start.begin(),
                              fov_middlepoint_path_part_goto_cell_start.end());
...
void BoustrophedonExplorer::downsamplePath(const std::vector<cv::Point>& original_path,
                                           std::vector<cv::Point>& downsampled_path, cv::Point& robot_pos,
...
void BoustrophedonExplorer::printCells(std::map<int, boost::shared_ptr<BoustrophedonCell> >& cell_index_mapping)
{
       ...
       itc != cell_index_mapping.end(); ++itc)
  {
    ...
    for (BoustrophedonCell::BoustrophedonCellSetIterator its = itc->second->neighbors_.begin();
         its != itc->second->neighbors_.end(); ++its)
      std::cout << (*its)->label_ << ", ";
    std::cout << std::endl;
  }
}
...
void BoustrophedonVariantExplorer::mergeCellsSelection(
    cv::Mat& cell_map, cv::Mat& cell_map_labels,
    std::map<int, boost::shared_ptr<BoustrophedonCell> >& cell_index_mapping, const double min_cell_area,
    const int min_cell_width)
{
...
// void BoustrophedonVariantExplorer::computeCellDecomposition(const cv::Mat& room_map, const float map_resolution,
// const double min_cell_area, 		std::vector<GeneralizedPolygon>& cell_polygons, std::vector<cv::Point>&
// polygon_centers)
//{
