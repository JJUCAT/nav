...
// Default constructor
GeneticTSPSolver::GeneticTSPSolver(int min_number_of_gens, int const_generations) : abort_computation_(false)
{
...
////Function to construct the distance matrix from the given points. See the definition at solveGeneticTSP for the style
///of this matrix.
// void GeneticTSPSolver::constructDistanceMatrix(cv::Mat& distance_matrix, const cv::Mat& original_map, const int
// number_of_nodes,
...
//					double length = pathplanner_.planPath(original_map, current_center, neighbor, downsampling_factor,
//robot_radius, map_resolution); 					pathlengths.at<double>(i, p) = length; 					pathlengths.at<double>(p, i) = length;
////symmetrical-Matrix --> saves half the computationtime
//				}
//			}
//			else
//			{
...
// This function calculates for a given path the length of it. The Path is a vector with ints, that show the index of
// the node in the path. It uses a pathlength Matrix, which should be calculated once. This Matrix should save the
// pathlengths with this logic:
...
// This Function takes the given path and mutates it. A mutation is a random change of the path-order. For example
// random nodes can be switched, or a random intervall of nodes can be inverted. Only the first and last Node can't be
// changed, because they are given from the Main-function.
std::vector<int> GeneticTSPSolver::mutatePath(const std::vector<int>& parent_path)
{
  ...
  if (what_to_change == 0)  // random node-switching
  {
    int number_of_switches =
        (rand() % (parent_path.size() - 3)) + 1;  // Set the number of switches that should be done.
    ...
    for (int change = 0; change < number_of_switches; change++)
    {
      ...
      do
      {
        int node_one = (rand() % (saving_variable_path.size() - 2)) +
                       1;  // this variables random choose which nodes should be changed
  ...
  else if (what_to_change == 1)               // random intervall-inverting (always from middle node on)
  {
    ...
    do
    {
      ...
      if (node_one > node_two)    // switch variables, if the node_one is bigger than the node_two (easier to work with
                                // here)
      {
...
// This Function calculates the length of each given path and chooses the shortest one. It uses the getPathLength
// function.
std::vector<int> GeneticTSPSolver::getBestPath(const std::vector<std::vector<int> > paths,
                                               const cv::Mat& pathlength_Matrix, bool& changed)
{
  std::vector<int> best_path = paths[0];

  double best_distance =
      getPathLength(pathlength_Matrix, paths[0]);  // saving-variable for the distance of the current best path
...
//	   of positions of nodes or the inversion of a intervall. The initial parent is the path from the nearest-neighbor
//search. 	II. It checks for the 9 paths (parent and children) for the best path (shortest) and takes this path as the
//parent 	   of the new generation. 	III. It repeats the steps I. and II. at least a specified amount of times and then
//checks if the pathlength 		 hasn't changed in the last steps.
//
...
std::vector<int> GeneticTSPSolver::solveGeneticTSP(const cv::Mat& path_length_Matrix, const int start_Node)
{
  ...
  calculated_path.push_back(start_Node);  // push the start node at the end, so the reaching of the start at the end is
                                          // included in the planning

  if (path_length_Matrix.rows > 2)  // check if graph has at least three members, if not the algorithm won't work
                                    // properly
  {
    bool changed_path = false;      // this variable checks if the path has been changed in the mutation process
    int changing_counter =
        const_generations_number_;  // this variable is a counter for how many times a path has been the same
    ...
    do
    {
      ...
      current_generation_paths.push_back(calculated_path);      // first path is always the parent --> important for
                                                            // checking if the path has changed in getBestPath!!
      ...
      calculated_path = getBestPath(current_generation_paths, path_length_Matrix,
                                    changed_path);              // get the best path of this generation
      if (number_of_generations >= min_number_of_generations_)  // when a specified amount of steps have been done the
                                                                // algorithm checks if the last paths didn't change
      {
        if (changed_path)
        {
...
std::vector<int> GeneticTSPSolver::solveGeneticTSP(const cv::Mat& original_map, const std::vector<cv::Point>& points,
                                                   double downsampling_factor, double robot_radius,
                                                   double map_resolution, const int start_Node,
                                                   cv::Mat* distance_matrix)
{
  ...
  boost::thread t(boost::bind(&GeneticTSPSolver::distance_matrix_thread, this, boost::ref(distance_matrix_computation),
                              boost::ref(distance_matrix_ref), boost::cref(original_map), boost::cref(points),
                              downsampling_factor, robot_radius, map_resolution, boost::ref(pathplanner_)));
  bool finished = false;
  ...
  //	DistanceMatrix distance_matrix_computation;
  //	distance_matrix_computation.constructDistanceMatrix(distance_matrix_ref, original_map, points,
  //downsampling_factor, robot_radius, map_resolution, pathplanner_);
...
std::vector<int> GeneticTSPSolver::solveGeneticTSPClean(const cv::Mat& original_map,
                                                        ...
                                                        double map_resolution, const int start_node)
{
  ...
  std::map<int, int> cleaned_index_to_original_index_mapping;  // maps the indices of the cleaned distance_matrix to the
                                                               // original indices of the original distance_matrix
  ...
  distance_matrix_computation.computeCleanedDistanceMatrix(original_map, points, downsampling_factor, robot_radius,
                                                           map_resolution, pathplanner_, distance_matrix_cleaned,
...
std::vector<int> GeneticTSPSolver::solveGeneticTSPWithCleanedDistanceMatrix(
    const cv::Mat& distance_matrix, const std::map<int, int>& cleaned_index_to_original_index_mapping,
