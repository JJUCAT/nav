...
std::vector<int> NearestNeighborTSPSolver::solveNearestTSP(const cv::Mat& path_length_matrix, const int start_node)
{
  ...
  if (path_length_matrix.rows > 1)  // check if clique has more than one member or else this algorithm produces a order
                                    // of size=3
  {
    ...
    } while (calculated_order.size() < path_length_matrix.rows);  // when the order has as many elements as the
                                                                  // pathlength Matrix has the solver is ready
  }
  else
  {
...
std::vector<int> NearestNeighborTSPSolver::solveNearestTSP(const cv::Mat& original_map,
...
std::vector<int> NearestNeighborTSPSolver::solveNearestTSPClean(const cv::Mat& original_map,
                                                                const std::vector<cv::Point>& points,
                                                                double downsampling_factor, double robot_radius,
                                                                double map_resolution, const int start_node)
{
