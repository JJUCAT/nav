...
// If the path from one node to another doesn't exist or the path is from one node to itself, the entry in the matrix
// must be 0 or smaller. so the format for this matrix is:
...
public:
  ...
  std::vector<int> solveNearestTSP(const cv::Mat& path_length_matrix,
                                   const int start_node);  // with given distance matrix

  // compute TSP and distance matrix without cleaning it
  // this version does not exclude infinite paths from the TSP ordering
  std::vector<int> solveNearestTSP(const cv::Mat& original_map, const std::vector<cv::Point>& points,
  ...
  std::vector<int> solveNearestTSPWithCleanedDistanceMatrix(
