...
// This class provides a solution for the TSP by taking the nearest-neighbor path and applying a genetic algorithm on
// it. It needs a symmetrical matrix of pathlenghts between the nodes and the starting-point index in this matrix. If
// the path from one node to another doesn't exist or the path is from one node to itself, the entry in the matrix must
...
protected:
  ...
  std::vector<int> getBestPath(const std::vector<std::vector<int> > paths, const cv::Mat& pathlength_Matrix,
                               bool& changed);
...
public:
  ...
  // Solving-algorithms for the given TSP. It returns a vector of int, which is the order from this solution. The int
  // shows the index in the Matrix. There are two functions for different cases:
  ...
  std::vector<int> solveGeneticTSP(const cv::Mat& original_map, const std::vector<cv::Point>& points,
                                   double downsampling_factor, double robot_radius, double map_resolution,
                                   const int start_Node, cv::Mat* distance_matrix = 0);
  ...
  std::vector<int> solveGeneticTSPWithCleanedDistanceMatrix(
