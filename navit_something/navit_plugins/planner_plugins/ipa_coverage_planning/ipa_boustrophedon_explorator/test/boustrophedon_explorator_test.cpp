...
{
  ros::init(argc, argv, "boustrophedon_explorator_test");
  ...
  ipa_boustrophedon_explorator::BoustrophedonExploratorRos planner;
  ...
  ros::Subscriber sub = nh.subscribe<geometry_msgs::PointStamped>(
      "/clicked_point", 1, [&](const geometry_msgs::PointStamped::ConstPtr& msg) {
        ...
        if (client.call(srv))
        {
          ...
          {
            auto& plogon = srv.response.selection.front();
            for (const auto& polygon : plogon.polygons)
            {
  ...
  auto timer = nh.createTimer(ros::Duration(0.002), [&](const ros::TimerEvent& event) {
    static int index = 0;
