...
void AStarPlanner::getRoute(const cv::Point start_point, const std::string& route, double step_length,
                                 ...
                                 const double robot_radius, const double map_resolution)
{
  ...
  if (downsampling_factor != 1.)
    cv::resize(eroded_map, downsampled_map, cv::Size(0, 0), downsampling_factor, downsampling_factor,
               cv::INTER_NEAREST);  //_LINEAR);
  else
...
// This is the path planning algorithm for this class. It downsamples the map with the given factor (0 < factor < 1) so
// the map gets reduced and calculation time gets better. If it is set to 1 the map will have original size, if it is 0
// the algorithm won't work, so make sure to not set it to 0. The algorithm also needs the Robot radius [m] and the map
// resolution [m²/pixel] to calculate the needed amount of erosions to include the radius in the planning.
                              ...
                              const int end_point_valid_neighborhood_radius, std::vector<cv::Point>* route)
{
  ...
  if (route_ == "")
  {
    ...
    if (route_ == "")
    {
      //			std::cout << "No path from " << start_point << " to " << end_point << " found for map of size " <<
      //map.rows << "x" << map.cols << " and downsampling factor " << downsampling_factor << std::endl;
...
double AStarPlanner::planPath(const cv::Mat& map, const cv::Mat& downsampled_map, const cv::Point& start_point,
                              ...
                              const double map_resolution, const int end_point_valid_neighborhood_radius,
                              cv::Mat* draw_path_map, std::vector<cv::Point>* route)
{
  route_ = "";
  double step_length = 1. / downsampling_factor;
  ...
  //	cv::waitKey();
  double pathlength =
      step_length * planPath(downsampled_map, downsampling_factor * start_point, downsampling_factor * end_point, 1.,
                             0., map_resolution, end_point_valid_neighborhood_radius, route);
