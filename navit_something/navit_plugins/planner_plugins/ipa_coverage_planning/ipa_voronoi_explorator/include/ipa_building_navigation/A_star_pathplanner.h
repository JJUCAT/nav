...
// This class provides an AStar pathplanner, which calculates the pathlength from one cv::Point to another. It was taken
// from
//  http://code.activestate.com/recipes/577457-a-star-shortest-path-algorithm/ and slightly changed (to use it with
//  openCV).
// It needs node.h to work, which gives an implementation of nodes of the graph.
//
//!!!!!!!!!Important!!!!!!!!!!!!!
// It downsamples the map mith the given factor (0 < factor < 1) so the map gets reduced and calculationtime gets
// better. If it is set to 1 the map will have original size, if it is 0 the algorithm won't work, so make sure to not
// set it to 0. The algorithm also needs the Robot radius [m] and the map resolution [m²/pixel] to calculate the needed
...
public:
  ...
  // computes the path length between start point and end point, tries first with a downsampled map for fast computation
  // and uses the original map if the first try was not successful if end_point_valid_neighborhood_radius [measured in
  // cell size of downsampled_map] is set greater than 0, then it is sufficient to find a path to a cell within that
  // neighborhood radius to end_point for a success
  double planPath(const cv::Mat& map, const cv::Mat& downsampled_map, const cv::Point& start_point,
                  const cv::Point& end_point, const double downsampling_factor, const double robot_radius,
                  const double map_resolution, const int end_point_valid_neighborhood_radius = 0,
                  cv::Mat* draw_path_map = NULL, std::vector<cv::Point>* route = NULL);
