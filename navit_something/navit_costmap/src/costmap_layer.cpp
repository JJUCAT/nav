...
void CostmapLayer::updateWithMax(navit_costmap::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  if (!enabled_)
    return;
...
void CostmapLayer::updateWithTrueOverwrite(navit_costmap::Costmap2D& master_grid, int min_i, int min_j,
                                           int max_i, int max_j)
{
  if (!enabled_)
    return;
...
void CostmapLayer::updateWithOverwrite(navit_costmap::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  if (!enabled_)
    return;
  unsigned char* master = master_grid.getCharMap();
...
void CostmapLayer::updateWithAddition(navit_costmap::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  if (!enabled_)
    return;
  unsigned char* master_array = master_grid.getCharMap();
  ...
  for (int j = min_j; j < max_j; j++)
  {
    ...
    for (int i = min_i; i < max_i; i++)
    {
      ...
      else
      {
        ...
        if (sum >= navit_costmap::INSCRIBED_INFLATED_OBSTACLE)
            master_array[it] = navit_costmap::INSCRIBED_INFLATED_OBSTACLE - 1;
        else
            master_array[it] = sum;
      }
      it++;
