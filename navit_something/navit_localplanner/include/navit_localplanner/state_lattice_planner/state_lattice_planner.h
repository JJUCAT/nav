...
public:
    ...
    public:
      ...
      Critic(
        const double dist_err, const double yaw_err, const double right_punishment,
        const double dist_scale, const double yaw_scale, const double right_punishment_scale) :
        dist_err_(dist_err), yaw_err_(yaw_err), right_punishment_(right_punishment),
        dist_scale_(dist_scale), yaw_scale_(yaw_scale), right_punishment_scale_(right_punishment_scale) {}
    ...
    private:
      ...
      double evaluate_yaw_err(
        const MotionModelDiffDrive::Trajectory& trajectory, const Eigen::Vector3d& goal);
      double evaluate_right_punishment(
        const MotionModelDiffDrive::Trajectory& trajectory, const Eigen::Vector3d& goal);

      double dist_err_;
      double yaw_err_;
      double right_punishment_;

      double dist_scale_;
      double yaw_scale_;
      double right_punishment_scale_;
    }; // Critic
    ...
    /**
     * @brief  sort trajectories by cost from low to high
     * @param  goal          goal on reference plan
     * @param  trajectories  trajectories to samples
     * @return true
     * @return false
     */
    bool sort_trajectories(const Eigen::Vector3d& goal, std::vector<MotionModelDiffDrive::Trajectory>& trajectories);
    /**
    ...
    /**
     * @brief  轨迹优化参数，区分参考路径上的回归采样和绕障采样
     *         回归采样的轨迹优化要比绕障采样的优化要求更加严格，可以分配更多优化时间
     * @param  ref_sampling_num 参考路径上的采样
     * @param  ref_timeout      回归采样优化的超时
     * @param  ref_dist_err     回归采样优化的终点欧式距离容错
     * @param  ref_yaw_err      回归采样优化的终点航向角容错
     * @param  avoid_timeout    绕障采样优化的超时
     * @param  avoid_dist_err   绕障采样优化的终点欧式距离容错
     * @param  avoid_yaw_err    绕障采样优化的终点航向角容错
     */
    void set_trajectory_config(const size_t ref_sampling_num,
      const double ref_timeout, const double ref_dist_err, const double ref_yaw_err,
      const double avoid_timeout, const double avoid_dist_err, const double avoid_yaw_err) {
      ref_sampling_num_ = ref_sampling_num;
      ref_opt_cfg_.set(ref_timeout, ref_dist_err, ref_yaw_err);
      avoid_opt_cfg_.set(avoid_timeout, avoid_dist_err, avoid_yaw_err);
    }

    /**
     * @brief  根据采样点位姿和当前速度，调整目标速度，效果不太好
     * @param  state            My Param doc
     * @param  target_v         My Param doc
     * @return double
     */
    double update_v(const Eigen::Vector3d state, const double target_v);
...
protected:
    ...
    struct trajectory_optimize_config_t {
      double timeout = 0.1;
      double min_dist_err = 0.1;
      double min_yaw_err = 0.17;
      void set(const double to, const double de, const double ye) {
        timeout = to;
        min_dist_err = de;
        min_yaw_err = ye;
      };
    };
    size_t ref_sampling_num_ = 0;
    trajectory_optimize_config_t ref_opt_cfg_;
    trajectory_optimize_config_t avoid_opt_cfg_;
