  ...
  const Eigen::Vector3d& goal, MotionModelDiffDrive::Trajectory& best_trajectory)
{
  ...
  for (size_t i = 0; i < trajectories.size(); i ++) {
    double cost = evaluate_trajectory(trajectories.at(i), goal);
    if (cost < cost_min) {
      cost_min = cost;
      cost_min_index = i;
    }
...
void StateLatticePlanner::Critic::sort_trajectories(
  ...
  std::map<double, MotionModelDiffDrive::Trajectory>& cost_trajectory_map)
{
  ...
  for (auto t : trajectories) {
    double cost = evaluate_trajectory(t, goal);
    cost_trajectory_map.insert({cost, t});
  }
}

double StateLatticePlanner::Critic::evaluate_trajectory(
  const MotionModelDiffDrive::Trajectory& trajectory, const Eigen::Vector3d& goal)
{
  double cost = 0;
  cost += evaluate_dist_err(trajectory, goal);
  cost += evaluate_yaw_err(trajectory, goal);
  cost += evaluate_right_punishment(trajectory, goal);
  return cost;
}
...
double StateLatticePlanner::Critic::evaluate_right_punishment(
  const MotionModelDiffDrive::Trajectory& trajectory, const Eigen::Vector3d& goal)
{
  double right_punishment_cost = 0.f;
  double yaw_diff = atan2(trajectory.trajectory.back()(1)-goal(1), trajectory.trajectory.back()(0)-goal(0)) - goal(2);
  if (yaw_diff > M_PI) yaw_diff -= 2*M_PI;
  else if (yaw_diff < -M_PI) yaw_diff += 2*M_PI;
  if (yaw_diff < goal(2)) right_punishment_cost = right_punishment_ * right_punishment_scale_;
  return right_punishment_cost;
}
  ...
  std::vector<MotionModelDiffDrive::Trajectory>& trajectories)
{
    ...
    for (size_t i=0;i<trajectory_num;i++) {
      ...
      ROS_DEBUG("[SLP] lookup msg, [%.3f,%.3f,%.3f] ,v:%.3f, k0:%.3f, km:%.3f, kf:%.3f, sf:%.3f",
        boundary_state(0), boundary_state(1), boundary_state(2), velocity, k0, param.omega.km, param.omega.kf, param.omega.sf);
      ...
      MotionModelDiffDrive::ControlParams init(
        ...
        // MotionModelDiffDrive::VelocityParams(velocity, MAX_ACCELERATION, v, v, MAX_ACCELERATION),
      ...
      if (i < ref_sampling_num_) {
        double timeout = ref_opt_cfg_.timeout - time_lapses;
        // std::cout << "ref init timeout:" << ref_opt_cfg_.timeout << ", lookup table speed:" << time_lapses << ", remain time:" << timeout << std::endl;
        if (timeout < 0.001) { timeout = 0.001; ROS_WARN("[SLP] less time for trajectory optimize."); }
        tg.set_optimize_config(timeout, ref_opt_cfg_.min_dist_err, ref_opt_cfg_.min_yaw_err);
      } else {
        double timeout = avoid_opt_cfg_.timeout - time_lapses;
        // std::cout << "avoid init timeout:" << avoid_opt_cfg_.timeout << ", lookup table speed:" << time_lapses << ", remain time:" << timeout << std::endl;
        if (timeout < 0.001) { timeout = 0.001; ROS_WARN("[SLP] less time for trajectory optimize."); }
        tg.set_optimize_config(timeout, avoid_opt_cfg_.min_dist_err, avoid_opt_cfg_.min_yaw_err);
      }
...
double StateLatticePlanner::update_v(const Eigen::Vector3d state, const double target_v)
{
