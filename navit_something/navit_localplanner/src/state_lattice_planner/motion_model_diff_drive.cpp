...
void MotionModelDiffDrive::make_velocity_profile(const double dt, const VelocityParams& v_param)
{
    ...
    ***************************************/
    ...
    // std::cout << "make v table, time:" << v_param.time << ", dt:" << dt << ", size:" << size << std::endl;
    ...
    for(int i=0;i<size;++i){
        ...
        if (ta > v_param.time) ta = v_param.time;
        // std::cout << "ta: " << ta << std::endl;
        // deceleration time
        double tf = fabs((v_param.vf - v_param.vt) / v_param.af);
        double td = v_param.time - tf;
        if (ta + tf > v_param.time) td = ta; // 没有匀速的时间
...
double MotionModelDiffDrive::estimate_driving_time(const ControlParams& control)
{
    ...
    if (st >= 0) { // 还有距离可以做匀速运动
      ...
      return driving_time;
    } else { // 距离太短不够做匀速运动，就只有加速运动或者是加速+减速运动
      ROS_DEBUG("[SLP] omega s is too short.");
      double driving_time = estimate_non_uniform_driving_time(control, t0 + td + 0.001);
      return driving_time;
    }
}
...
double MotionModelDiffDrive::estimate_non_uniform_driving_time(const ControlParams& control, const double t_max)
{
  double t0 = fabs((control.vel.vt - control.vel.v0) / control.vel.a0);
  double s0 = 0.5 * fabs(control.vel.vt + control.vel.v0) * t0;
  ...
  // 距离不够达到匀速速度
  // 0.5 * a * t^2 + v0 * t - s = 0
  if (s0 <= control.omega.sf) {
    double t = std::numeric_limits<double>::max();
    double delta = control.vel.v0 * control.vel.v0 - 4 * 0.5 * control.vel.a0 * (-control.omega.sf);
    if (delta < 0) t = t_max;
    else if (delta == 0) {
      double t0 = -control.vel.v0 / 2 / 0.5 / control.vel.a0;
      if (t0 > 0 && t0 < t) t = t0;
    } else {
      double t0 = (-control.vel.v0 + std::sqrt(delta)) / 2 / 0.5 / control.vel.a0;
      if (t0 > 0 && t0 < t) t = t0;
      double t1 = (-control.vel.v0 - std::sqrt(delta)) / 2 / 0.5 / control.vel.a0;
      if (t1 > 0 && t1 < t) t = t1;
    }
    t = std::min(t_max, t);
    return t;
  ...
  // 剩下的距离做减速运动
  double s1 = control.omega.sf - s0;
  double t = std::numeric_limits<double>::max();
  double delta = control.vel.vt * control.vel.vt - 4 * 0.5 * control.vel.a0 * (-s1);
  if (delta < 0) t = t_max;
  else if (delta == 0) {
    double t0 = -control.vel.vt / 2 / 0.5 / control.vel.a0;
    if (t0 > 0 && t0 < t) t = t0;
  } else {
    double t0 = (-control.vel.vt + std::sqrt(delta)) / 2 / 0.5 / control.vel.a0;
    if (t0 > 0 && t0 < t) t = t0;
    double t1 = (-control.vel.vt - std::sqrt(delta)) / 2 / 0.5 / control.vel.a0;
    if (t1 > 0 && t1 < t) t = t1;
  }
  t = std::min(t_max, t);
  return t;
