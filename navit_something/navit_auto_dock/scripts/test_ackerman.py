...
def doneCallback(state, result):
    ...
    if approach_res.reached:
       ...
       re_app=Bool()
       re_app.data=True
       approach_result_pub.publish(re_app)
    else :
    ...
    goal_.dock_pose.pose.position.x = feed_back.pose.position.x+(math.cos(theta)* -0.6)
    goal_.dock_pose.pose.position.y = feed_back.pose.position.y+(math.sin(theta)* -0.6)
...
if __name__ == "__main__":
    ...
    final_result_pub = rospy.Publisher("/final_action_succeed",Bool,queue_size=10)
    approach_result_pub = rospy.Publisher("/approach_action_succeed",Bool,queue_size=10)
