#ifndef LINEAR_POSE_FILTER_H
#define LINEAR_POSE_FILTER_H
...
public:
  ...
  /**
   * @brief Method to filter pose objects. Filter only filters position.x,
   * position.y and the
   *        yaw of the pose.
  ...
  * @param input_pose  The initial input pose that will be set for all previous
  * inputs.
  * @param output_pose The initial output pose that will be set for all previous
  * outputs.
  */
  ...
  * state(s).
  *                     new_poses[0] is the oldest sample and
  * new_poses[new_poses.size() - 1]
  *                     is the newest sample.
  *                     If the number of poses is greater than the length of the
  * filter memory,
  *                     only the newest poses will be used.
  *                     If the number of poses is less than the length of the
  * filter memory,
  *                     the poses will be copied to the most recent history and
  * the original
  *                     samples will retain their original location in the
  * buffer.
  *                     Example:
  *                     // New poses.
  *                     np = [1, 2, 3]
  ...
  *                     p  = [9, 8, 1, 2, 3]
  * @param input_poses  Set of input poses to set the input state time history
  * to.
  * @param output_poses Set of output poses to set the output state time history
  * to.
  */
   ...
   * histories and then
   *        recreate the appropriate output histories. This is because the input
   * history encodes the
...
private:
   ...
   * @return Pose with the linear pose set to the origin and the quaternion set
   * to
   ...
   *
   *        This method adds shortest angular distance between the pose and the
   * reference
   *        yaw and returns it.
   *        Example:
   *          pose = -178 degrees, reference_yaw = 177 degrees, return = 182
   * degrees.
   *          pose =  175 degrees, reference_yaw = -170 degrees, return = -185
   * degrees.
  ...
  std::deque<float>
      yaw_out_; /// Vector of previous output yaws that aren't normalized.
  std::deque<float>
      yaw_in_; /// Vector of previous input yaws that aren't normalized.
