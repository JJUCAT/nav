...
class NTRIPRos:
  def __init__(self):
    ...
    self._rtk_input_port = rospy.get_param('~rtk_input_port', '/dev/rtk_input')
    self._rtk_input = None
    self._without_rtk = rospy.get_param('~without_rtk', False)
  ...
  def run(self):
    ...
    if not self._without_rtk:
        # 初始化RTK模块的串口，波特率115200，
        # RTK输出GPGGA数据，发送到ntrip服务器
        # 把从ntrip服务器返回的RTCM数据，发送到RTK模块
        # 如果初始化串口失败，则循环尝试
        while rospy.is_shutdown() is False:
            try:
                self._rtk_input = serial.Serial(self._rtk_input_port, 115200, timeout=0.1)
                break
            except serial.SerialException as e:
                rospy.logfatal('Failed to open RTK input port: %s', e)
                rospy.logwarn('Please check the port and permissions')
                rospy.sleep(1.0)

        # Start the timer that will check for RTCM data
        self._rtcm_timer = rospy.Timer(rospy.Duration(0.1), self.publish_rtcm)

        # 再做一个定时器，每隔1秒钟，从RTK模块的串口读取数据，发送到ntrip服务器
        self._nmea_timer = rospy.Timer(rospy.Duration(0.1), self.publish_nmea)
    
    # Start the timer that will check for RTCM data
    self._rtcm_timer = rospy.Timer(rospy.Duration(0.1), self.publish_rtcm_without_rtk)
  ...
  def stop(self):
    ...
    rospy.loginfo('Stopping NMEA publisher')
    if self._nmea_timer:
      self._nmea_timer.shutdown()
      self._nmea_timer.join()
  ...
  def publish_nmea(self, event):
    # 从RTK模块的串口读取数据，发送到ntrip服务器
    nmea = self._rtk_input.readline()
    self._client.send_nmea(nmea)

  def publish_rtcm(self, event):
    for raw_rtcm in self._client.recv_rtcm():
      self._rtcm_pub.publish(self._create_rtcm_message(raw_rtcm))
      self._rtk_input.write(raw_rtcm)

  def publish_rtcm_without_rtk(self, event):
    for raw_rtcm in self._client.recv_rtcm():
      self._rtcm_pub.publish(self._create_rtcm_message(raw_rtcm))
