#include <GnssParserLib/novatel_message_extractor.h>
#include <GnssParserLib/parsers/gpgga.h>
#include <GnssParserLib/parsers/gprmc.h>
...
uint32_t NovatelMessageExtractor::CalculateBlockCRC32(uint32_t ulCount,         // Number of bytes in the data block
...
void NovatelMessageExtractor::VectorizeString(const std::string& str, std::vector<std::string>& vectorized_message,
                                              const std::string& delimiters) {
    boost::algorithm::split(vectorized_message, str, boost::algorithm::is_any_of(delimiters));
}

bool NovatelMessageExtractor::GetNovatelMessageParts(const std::string& sentence, std::string& message_id,
...
int32_t NovatelMessageExtractor::GetBinaryMessage(const std::string& str, size_t start_idx, BinaryMessage& msg) {
    ...
    if (msg.header_.header_length_ != HeaderParser::BINARY_HEADER_LENGTH) {
        ROS_WARN("Binary header length was unexpected: %u (expected %u)", msg.header_.header_length_,
                 HeaderParser::BINARY_HEADER_LENGTH);
    }

    ROS_DEBUG("Msg ID: %u    Data start / length: %u / %u", msg.header_.message_id_, data_start, data_length);
...
int32_t NovatelMessageExtractor::GetNovatelSentence(const std::string& str, size_t start_idx, std::string& sentence) {
    sentence.clear();
    ...
    if (checksum_start == std::string::npos) {
        ...
        uint64_t calculated_checksum = CalculateBlockCRC32(static_cast<uint32_t>(sentence.size()),
...
int32_t NovatelMessageExtractor::GetNmeaSentence(const std::string& str, size_t start_idx, std::string& sentence,
                                                 bool keep_container) {
    sentence.clear();
    ...
    if (checksum_start == std::string::npos) {
        ...
        if (checksum == ULONG_MAX) {
            ...
            if (keep_container) {
                ...
                sentence.insert(sentence.end(), recreated_checksum_str.begin(), recreated_checksum_str.end());
            }
            return 0;
        } else {
...
void NovatelMessageExtractor::FindAsciiSentence(const std::string& sentence, size_t current_idx, size_t& start_idx,
                                                size_t& end_idx, size_t& invalid_char_idx) {
...
bool NovatelMessageExtractor::VectorizeNovatelSentence(const std::string& data, NovatelSentence& sentence) {
    return GetNovatelMessageParts(data, sentence.id, sentence.header, sentence.body);
}

void NovatelMessageExtractor::VectorizeNmeaSentence(const std::string& sentence, NmeaSentence& vectorized_message) {
...
bool NovatelMessageExtractor::ExtractCompleteMessages(const std::string& input,
    ...
    while (sentence_start != std::string::npos && sentence_start < input.size()) {
        ...
        ROS_DEBUG("Binary start: %lu   ASCII start / end / invalid: %lu / %lu / %lu", binary_start_idx, ascii_start_idx,
                  ascii_end_idx, invalid_ascii_idx);
        ...
        if (ascii_start_idx == std::string::npos ||
            ...
            if (result > 0) {
                ...
                ROS_WARN("Binary message was incomplete, waiting for more.");
                break;
            } else {
            ...
            if (invalid_ascii_idx != std::string::npos) {
                ...
                if (input[ascii_start_idx] == NMEA_SENTENCE_FLAG[0]) {
                    ...
                    int32_t result = GetNmeaSentence(input, ascii_start_idx, cur_sentence, keep_nmea_container);
                    if (result == 0) {
                        ...
                        ROS_WARN("Invalid NMEA checksum for: [%s]", input.substr(ascii_start_idx, ascii_len).c_str());
                        // Sentence had an invalid checksum, just iterate to the next sentence
                        sentence_start += 1;
                        parse_error = true;
                    }
