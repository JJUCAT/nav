#include <GnssParserLib/parsers/bestpos.h>
...
navit_msgs::NovatelPositionPtr BestposParser::ParseBinary(const BinaryMessage &bin_msg) noexcept(false) {
    if (bin_msg.data_.size() != BINARY_LENGTH) {
        std::stringstream error;
        error << "Unexpected BESTPOS message length: " << bin_msg.data_.size();
        throw ParseException(error.str());
    }
    navit_msgs::NovatelPositionPtr ros_msg = boost::make_shared<navit_msgs::NovatelPosition>();
...
navit_msgs::NovatelPositionPtr BestposParser::ParseAscii(const NovatelSentence &sentence) noexcept(false) {
    navit_msgs::NovatelPositionPtr msg = boost::make_shared<navit_msgs::NovatelPosition>();
