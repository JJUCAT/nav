...
void GetSignalsUsed(uint32_t mask, navit_msgs::NovatelSignalMask &msg) {
    ...
    msg.gps_L5_used_in_solution     = mask & 0x04u;
    msg.BDS_B1_used_in_solution     = mask & 0x08u;
    msg.glonass_L1_used_in_solution = mask & 0x10u;
    msg.glonass_L2_used_in_solution = mask & 0x20u;
    msg.BDS_B2_used_in_solution     = mask & 0x40u;
    msg.BDS_B3_used_in_solution     = mask & 0x80u;
}
...
bool ParseDouble(const std::string &string, double &value) { return ToDouble(string, value) || string.empty(); }
...
bool ParseFloat(const std::string &string, float &value) { return ToFloat(string, value) || string.empty(); }
...
bool ParseInt16(const std::string &string, int16_t &value, int32_t base) {
    ...
    int32_t tmp;
    if (ToInt32(string, tmp, base) && tmp <= std::numeric_limits<int16_t>::max() &&
...
bool ParseInt32(const std::string &string, int32_t &value, int32_t base) {
    return ToInt32(string, value, base) || string.empty();
}
...
bool ParseUInt32(const std::string &string, uint32_t &value, int32_t base) {
    return ToUInt32(string, value, base) || string.empty();
}
...
bool ParseUInt8(const std::string &string, uint8_t &value, int32_t base) {
    ...
    uint32_t tmp;
    if (ToUInt32(string, tmp, base) && tmp <= std::numeric_limits<uint8_t>::max()) {
...
bool ParseUInt16(const std::string &string, uint16_t &value, int32_t base) {
    ...
    uint32_t tmp;
    if (ToUInt32(string, tmp, base) && tmp <= std::numeric_limits<uint16_t>::max()) {
...
double UtcFloatToSeconds(double utc_float) {
    ...
    double seconds   = utc_float - static_cast<double>(hours * 10000 + minutes * 100);
