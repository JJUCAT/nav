#include <GnssParserLib/parsers/gpgsa.h>

#include <boost/make_shared.hpp>

const std::string integrated_navigation::GpgsaParser::MESSAGE_NAME = "GPGSA";

uint32_t integrated_navigation::GpgsaParser::GetMessageId() const { return 0; }

const std::string integrated_navigation::GpgsaParser::GetMessageName() const { return MESSAGE_NAME; }

navit_msgs::GpgsaPtr integrated_navigation::GpgsaParser::ParseAscii(
    ...
    if (sentence.body.size() != LENGTH) {
        ...
        error << "Expected GPGSA length " << LENGTH << ", actual length " << sentence.body.size();
        throw ParseException(error.str());
    }

    navit_msgs::GpgsaPtr msg = boost::make_shared<navit_msgs::Gpgsa>();
    ...
    size_t n_svs = 0;
    for (std::vector<std::string>::const_iterator id = sentence.body.begin() + 3; id < sentence.body.begin() + 15;
         ++id) {
        if (!id->empty()) {
