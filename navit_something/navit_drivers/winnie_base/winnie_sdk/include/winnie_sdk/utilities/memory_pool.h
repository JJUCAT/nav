#ifndef WINNIE_SDK_MEMORY_POOL_H
#define WINNIE_SDK_MEMORY_POOL_H
...
class MemoryPool {
   public:
     ...
     */
    MemoryPool(uint16_t max_block_size = 1024, uint16_t memory_size = 1024, uint16_t memory_table_num = 32)
        : memory_size_(memory_size), memory_table_num_(memory_table_num), max_block_size_(max_block_size){};
    ...
    MemoryBlock *AllocMemory(uint16_t size) {
        ...
        for (i = 0; i < (memory_table_used_num - 1); i++) {
            ...
            // Check if block left memory size is enough for needed size, if so, then record block table id and its left
            // memory
...
#endif  // WINNIE_SDK_MEMORY_POOL_H
