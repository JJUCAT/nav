#ifndef WINNIE_SDK_PROTOCOL_H
#define WINNIE_SDK_PROTOCOL_H
...
enum class ACKSessionStatus : uint8_t {
    ...
    ACK_SESSION_PROCESS =
        1,  ///< ack session is in process of dealing with ack feedback after receiving ack needed command
...
typedef struct CMDSession {
    ...
    //! previous sending sequence number
...
class Protocol {
   public:
     ...
     */
    explicit Protocol(std::shared_ptr<HardwareInterface> device_ptr);
    ...
    /**
     * @brief An interface function for dispatch layer to take the message/package from the circular buffer in the
     * protocol layer
     ...
     *         input command information mismatches the information with the same command set and id already in the
     * circular buffer
     */
    bool Take(const CommandInfo *command_info, MessageHeader *message_header, void *message_data);
     ...
     */
    bool SendResponse(const CommandInfo *command_info, const MessageHeader *message_header, void *message_data);
     ...
     */
    bool SendRequest(const CommandInfo *command_info, MessageHeader *message_header, void *message_data);
     ...
     * @return True if command is successfully allocated and sent by protocol layer
     */
    bool SendMessage(const CommandInfo *command_info, void *message_data);
    /*************************** Send Pipline ***************************/
    /**
     ...
     */
    bool SendCMD(uint8_t cmd_set, uint8_t cmd_id, uint8_t receiver, void *data_ptr, uint16_t data_length,
     ...
     */
    bool SendACK(uint8_t session_id, uint16_t seq_num, uint8_t receiver, void *ack_ptr, uint16_t ack_length);
    ...
    /*************************** Recv Pipline ***************************/
    /**
     * @brief Get the byte from the device receive interface into the receive buffer and put it into byte handler,
     * return if a full frame is got
    ...
    /**
     * @brief Input the byte from the byte handler, put it into the reuse stream and check if the stream valid for a
     * frame
     ...
     * @details Validate the sof, version, receiver, length and header crc. If success, just go ahead. If not, shift the
     * stream
     ...
     * @details Validate the data crc for whole package. If success, just put it into conatiner handler and then prepare
     * the stream. If not, reuse the stream
     * @return True if a full frame is got
     */
    bool VerifyData();
   ...
   private:
    ...
    std::shared_ptr<HardwareInterface> device_ptr_ = nullptr;
