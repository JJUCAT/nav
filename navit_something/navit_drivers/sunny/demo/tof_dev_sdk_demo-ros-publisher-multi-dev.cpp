...
ros::Publisher g_pub_info[MAX_DEV_COUNT];
ros::Publisher g_pub_pointCloud[MAX_DEV_COUNT];
ros::Publisher g_pub_depthz[MAX_DEV_COUNT];
ros::Publisher g_pub_gray[MAX_DEV_COUNT];
ros::Publisher g_pub_rgbd[MAX_DEV_COUNT];
ros::Publisher g_pub_rgb[MAX_DEV_COUNT];

sensor_msgs::CameraInfoPtr g_camInfoMsg[MAX_DEV_COUNT];
sensor_msgs::PointCloud2 g_cloudMsg[MAX_DEV_COUNT];
sensor_msgs::ImagePtr g_depthzMsg[MAX_DEV_COUNT];
sensor_msgs::ImagePtr g_grayMsg[MAX_DEV_COUNT];
sensor_msgs::ImagePtr g_rgbdMsg[MAX_DEV_COUNT];
sensor_msgs::ImagePtr g_rgbMsg[MAX_DEV_COUNT];

pcl::PointCloud<pcl::PointXYZ> g_pclCloud[MAX_DEV_COUNT];
...
typedef struct tagChosedDevAndMode
{
	UINT32 devIndex;
	HTOFD hTofD;
	TofDeviceInfo struCaps;
...
static const char* StringColorFormat(const COLOR_FORMAT type)
{
	...
	switch (type)
	{
	case COLOR_FORMAT_MJPEG: pStr = "JPG"; break;
...
static bool SavePixelCoordText(PixelCoordData *pPixelCoord, const unsigned int width, const unsigned int height, char* pTxtFile)
{
...
static void publishPointCloud(TofFrameData *tofFrameData, StreamCallBackParam *cbData)
{
	...
	pcl::PointCloud<pcl::PointXYZ> &pclCloud = g_pclCloud[cbData->dev_index];
	sensor_msgs::PointCloud2 &cloudMsg = g_cloudMsg[cbData->dev_index];
	ros::Publisher &pub_pointCloud = g_pub_pointCloud[cbData->dev_index];
	...
	std::string frameId = "camera_point_cloud_frame" + std::to_string(cbData->dev_index);
...
static void publishDepthzImage(TofFrameData *tofFrameData, StreamCallBackParam *cbData)
{
	...
	sensor_msgs::ImagePtr &depthzMsg = g_depthzMsg[cbData->dev_index];
	ros::Publisher &pub_depthz = g_pub_depthz[cbData->dev_index];
	...
	std::string frameId = "camera_depthz_frame" + std::to_string(cbData->dev_index);
...
static void publishGrayImage(TofFrameData *tofFrameData, StreamCallBackParam *cbData)
{
	...
	sensor_msgs::ImagePtr &grayMsg = g_grayMsg[cbData->dev_index];
	ros::Publisher &pub_gray = g_pub_gray[cbData->dev_index];
	...
	std::string frameId = "camera_gray_frame" + std::to_string(cbData->dev_index);
...
static void publishRgbdImage(TofFrameData *tofFrameData, StreamCallBackParam *cbData)
{
	...
	sensor_msgs::ImagePtr &rgbdMsg = g_rgbdMsg[cbData->dev_index];
	ros::Publisher &pub_rgbd = g_pub_rgbd[cbData->dev_index];
	...
	std::string frameId = "camera_rgbd_frame" + std::to_string(cbData->dev_index);
...
static void publishRgbImage(RgbFrameData *rgbFrameData, StreamCallBackParam *cbData)
{
	...
	sensor_msgs::ImagePtr &rgbMsg = g_rgbMsg[cbData->dev_index];
	ros::Publisher &pub_rgb = g_pub_rgb[cbData->dev_index];

	rgbMsg = cv_bridge::CvImage(std_msgs::Header(), sensor_msgs::image_encodings::BGR8, rgbMat).toImageMsg();
	std::string frameId = "camera_rgb_frame" + std::to_string(cbData->dev_index);
...
static TOF_MODE ChoseTofMode(const UINT32 devIndex, TofDeviceInfo& struCaps)
{
	//打印出所有支持的TOF模式，供用户选择
...
static void ThreadPublishCameraInfo(HTOFD hTofD, TofDeviceCapability *pCaps, UINT32 devIndex)
{
	TOFRET retVal = TOFRET_ERROR_OTHER;
	...
	sensor_msgs::CameraInfoPtr &camInfoMsg = g_camInfoMsg[devIndex];
	ros::Publisher &pub_info = g_pub_info[devIndex];
...
static void StartDeviceTask(ChosedDevAndMode* pDev)
{
	TOFRET retVal = TOFD_SetTofMode(pDev->hTofD, pDev->tofMode);
	if (TOFRET_SUCCESS != retVal)
	{
		printf("dev %d, set tof mode (0x%08x), [ FAILED ], retVal=0x%08x!!!!!!!!!!!!!!!!!!!!!\n\n", pDev->devIndex, pDev->tofMode, retVal);
		return;
	}

	TofDeviceCapability* pCaps = GetTofModeCaps(pDev->struCaps, pDev->tofMode);//获取tof mode对应的能力

	//
	StreamCallBackParam& streamParam = pDev->streamParam;
	memset(&streamParam, 0, sizeof(streamParam));
	memcpy(&streamParam.struCaps, &(pDev->struCaps), sizeof(pDev->struCaps));
	streamParam.dev_index = pDev->devIndex;
	const bool bSuc = OpenStream(pDev->hTofD, pCaps, &streamParam);
	if (bSuc)
	{
		//成功后，等到数据流拿到后再去设置参数，这样才能保证生效(此逻辑是为了兼容所有模块)
		while ((0 >= streamParam.tof_frame_count)
			&& (0 >= streamParam.rgb_frame_count))
		{
			printf("dev %d, Waiting for stream data...\n", pDev->devIndex);
			std::this_thread::sleep_for(std::chrono::milliseconds(1000)); //单位是毫秒
		}

		printf("dev %d, Get Or Set Some Params...\n", pDev->devIndex);
		GetOrSetSomeParam(pDev->hTofD, pCaps);
	}

	//线程里定时发布camerainfo,需要在开流之后才行
	std::thread thread_publish_camera_info = std::thread(ThreadPublishCameraInfo, pDev->hTofD, pCaps, pDev->devIndex);
	thread_publish_camera_info.detach();
}
static void StopDeviceTask(ChosedDevAndMode* pDev, std::string strSaveDir)
{
	TofDeviceCapability* pCaps = GetTofModeCaps(pDev->struCaps, pDev->tofMode);//获取tof mode对应的能力

	CloseStream(pDev->hTofD, pCaps);

	if ("" != strSaveDir)
	{
		SaveSomeData(pDev->hTofD, &(pDev->struCaps), strSaveDir);//保存一些文件，便于问题排查
	}

	TOFD_CloseDevice(pDev->hTofD);

}

static void DoTestDemo(ChosedDevAndMode* pDevsList, const UINT32 dev_cnt, ros::NodeHandle& nh, std::string& strSaveDir)
{
	for (UINT32 devIndex = 0; devIndex < dev_cnt; devIndex++)
	{
		TofDeviceCapability* pCaps = GetTofModeCaps(pDevsList[devIndex].struCaps, pDevsList[devIndex].tofMode);//获取tof mode对应的能力

		//init camera info publisher
		std::string tempStr = "sunny_topic/device" + std::to_string(devIndex);
		g_pub_info[devIndex] = nh.advertise<sensor_msgs::CameraInfo>((tempStr + "/camera_info").c_str(), 5);
		g_camInfoMsg[devIndex].reset(new sensor_msgs::CameraInfo);

		if (pCaps->bTofSupported)
		{
			//init point clouds publisher
			g_pub_pointCloud[devIndex] = nh.advertise<sensor_msgs::PointCloud2>((tempStr + "/tof_frame/pointcloud").c_str(), 5);

			//init depthz publisher
			g_pub_depthz[devIndex] = nh.advertise<sensor_msgs::Image>((tempStr + "/tof_frame/depthz").c_str(), 5);
			g_depthzMsg[devIndex].reset(new sensor_msgs::Image);

			//init gray publisher
			g_pub_gray[devIndex] = nh.advertise<sensor_msgs::Image>((tempStr + "/tof_frame/gray").c_str(), 5);
			g_grayMsg[devIndex].reset(new sensor_msgs::Image);

		}

		if (pCaps->bRgbDSupported)
		{
			//init rgbd publisher
			g_pub_rgbd[devIndex] = nh.advertise<sensor_msgs::Image>((tempStr + "/tof_frame/rgbd").c_str(), 5);
			g_rgbdMsg[devIndex].reset(new sensor_msgs::Image);
		}

		if (pCaps->bRgbSupported)
		{
			//init rgb publisher
			g_pub_rgb[devIndex] = nh.advertise<sensor_msgs::Image>((tempStr + "/rgb_frame/rgb").c_str(), 5);
			g_rgbMsg[devIndex].reset(new sensor_msgs::Image);
		}

		StartDeviceTask(pDevsList + devIndex);

		//延时主要是为了让两个模块错开曝光时刻，不然叠加后瞬间电流太大会起不来，但是因无法掌握准确时间，因此很有可能是没有错开的
		std::this_thread::sleep_for(std::chrono::milliseconds(330)); //单位是毫秒
	}
	...
	for (UINT32 devIndex = 0; devIndex < dev_cnt; devIndex++)
	{
		StopDeviceTask(pDevsList + devIndex, strSaveDir);
	}

}


static bool ChoseDevTofMode(TofDeviceDescriptor* pDevsDesc, const UINT32 devIndex, ChosedDevAndMode& chosedDev)
{
	...
	chosedDev.tofMode = ChoseTofMode(devIndex, struCaps);//选择其中一种TOF模式出TOF数据

	return true;
}


static UINT32 OpenAllDevAndChoseTofMode(TofDeviceDescriptor* pDevsDescList, const UINT32 dev_cnt, ChosedDevAndMode* pDevsList)
{
	ChosedDevAndMode struTmp;
	UINT32 suc_cnt = 0;
	...
	for (UINT32 devIndex = 0; devIndex < dev_cnt; devIndex++)
	{
		...
		memcpy(pDevsList + suc_cnt, &struTmp, sizeof(struTmp));
		suc_cnt++;
	}
...
/*
用例功能简述：单线程下依次逐个打开多个设备进行取流（串行方式），设备间可能同时出流，适用linux下的ROS系统。
*/
int main(int argc, char **argv)
{
	...
	if (0 < searched_dev_cnt)
	{
		std::unique_ptr<ChosedDevAndMode[]> devsList(new ChosedDevAndMode[searched_dev_cnt]);
		const UINT32 suc_dev_cnt = OpenAllDevAndChoseTofMode(pDevsDescList, searched_dev_cnt, devsList.get());//打开每个设备，并选择每个设备使用的模式
		DoTestDemo(devsList.get(), suc_dev_cnt, node_handle, strSaveDir);
	}
	else
	{
