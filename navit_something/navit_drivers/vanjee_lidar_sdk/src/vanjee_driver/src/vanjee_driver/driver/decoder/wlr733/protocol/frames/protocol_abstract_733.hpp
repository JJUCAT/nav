...
namespace vanjee
{
  namespace lidar
  {
    class ProtocolAbstract733:public ProtocolAbstract
    {
      public:
        ProtocolAbstract733(const std::shared_ptr<CmdClass> sp_cmd,std::shared_ptr<ParamsAbstract> content,uint16 idx = 0,uint32 timestamp = 0,
          uint8 checkType = 1,uint8 type = 1):ProtocolAbstract(checkType,type,{0x00,0x10},
        ...
        virtual std::shared_ptr<std::vector<uint8>> GetRequest(std::shared_ptr<std::vector<uint8>> content = nullptr) override
        {
          ...
          return (std::make_shared<ProtocolBase>(ByteVector({0x00,0x00,0x00,0x00}),ByteVector({0x00,0x00,0x00,0x00}),CheckType,Type,
