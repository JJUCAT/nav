/*
 * @Author: guo
 * @Date: 2023-01-31 17:37:55
 * @LastEditors: Guo Xuemei 1242143575@qq.com
 * @LastEditTime: 2023-03-01 18:52:12
 * @FilePath: /vanjee_lidar_sdk_test/src/vanjee_lidar_sdk/src/vanjee_driver/src/vanjee_driver/driver/decoder/chan_angles.hpp
 */
#pragma once
...
#pragma pack(pop)
        ...
        class Vanjee739ChanAngles
        {
        private:
            std::vector<std::vector<float>> Vanjee739vertical_angles_;
            std::vector<std::vector<float>> Vanjee739horiz_angles_;
            uint16_t row_;    
            uint16_t column_; 
            uint16_t rpm_;
        public:
            std::vector<std::vector<float>> init(uint16_t row, uint16_t column)
            {
            ...
            Vanjee739ChanAngles(uint16_t rpm)
            :rpm_(rpm)
            {
                std::cout << "rpm_" << rpm_ << std::endl;
                if(rpm == 600)
                {
                    row_ = 128;
                    column_ = 850;
                ...
                Vanjee739vertical_angles_= init(row_,column_);
                Vanjee739horiz_angles_ = init(row_,column_);

            }
            int loadVanjee739Vertical(const std::string &angle_path)
            {
                std::vector<std::vector<float>> vert_angles;
                int ret = loadHorizontalangles(angle_path, vert_angles);
                if (ret < 0)
                    return ret;

                Vanjee739vertical_angles_.swap(vert_angles);
                return 0;
            }
            int loadVanjee739Horizontal(const std::string &angle_path)
            {
                std::vector<std::vector<float>> hor_angles;
                int ret = loadHorizontalangles(angle_path, hor_angles);
                if (ret < 0)
                    return ret;

                Vanjee739horiz_angles_.swap(hor_angles);
                return 0;
            }
            float Vanjee739VerticalAdjust(uint16_t row, uint16_t column)
            {
                return (Vanjee739vertical_angles_.at(row).at(column));
            }

            float Vanjee739horizAdjust(uint16_t row, uint16_t column)
            {
                return (Vanjee739horiz_angles_.at(row).at(column));
            }
        private:
            static int loadHorizontalangles(const std::string& angle_path, std::vector<std::vector<float>>& Vanjee739_angles_)
            {
                std::ifstream fd(angle_path.c_str(), std::ios::in);
                if (!fd.is_open())
                {
                    std::cout << "fail to open angle filehor:" << angle_path << std::endl;
                    return -1;
                }
                std::string h_line;
                std::vector<float> h_p_pointNum;
                while (std::getline(fd, h_line))
                {
                    std::stringstream sinFile(h_line);
                    std::string h_field;
                    float horiz = 0.f;
                    while (std::getline(sinFile, h_field, ','))
                    {
                        horiz = std::atof(h_field.c_str());
                        float tem_horiz= horiz * 1000.0;

                        h_p_pointNum.emplace_back(static_cast<float>(tem_horiz));
                    }
                    Vanjee739_angles_.emplace_back(h_p_pointNum);
                    h_p_pointNum.clear();
                }
