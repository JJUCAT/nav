...
SerialConfig::SerialConfig(int32_t baud, int32_t data_bits, int32_t stop_bits, Parity parity, bool flow_control,
                           bool low_latency_mode, bool writable)
  : baud(baud)
  , data_bits(data_bits)
  , stop_bits(stop_bits)
  , parity(parity)
...
SerialPort::SerialPort() : fd_(-1), error_msg_("")
{
}
...
bool SerialPort::Open(const std::string& device, SerialConfig config)
{
  ...
  if (config.parity != SerialConfig::NO_PARITY && config.parity != SerialConfig::EVEN_PARITY &&
...
SerialPort::Result SerialPort::ReadBytes(std::string& output, size_t max_bytes, int32_t timeout)
{
  if (fd_ < 0)
  {
    error_msg_ = "Device not open.";
...
int32_t SerialPort::Write(const std::string& input)
{
