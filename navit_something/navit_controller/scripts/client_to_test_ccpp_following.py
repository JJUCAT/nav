...
def feedbackCallback(result):
    if(result.completion_percentage>2.0):
        re=Bool(0)
        re.data=True
        result_pub.publish(re)
...
def doneCallback(state, result):
    ...
    controller_client.send_goal(controller_action_goal, done_cb=controlDoneCallback, active_cb=None, feedback_cb=feedbackCallback)
...
if __name__ == "__main__":
    ...
    localization_pub = rospy.Publisher("/initialpose",PoseWithCovarianceStamped,queue_size = 5)
