...
planner_path_pub = rospy.Publisher("global_path", Path, queue_size=1)
    ...
    def result_callback(self, state, result):
        if state == actionlib.GoalStatus.SUCCEEDED:
            ...
            if result.path_found == True:
                ...
                rospy.loginfo("recv global path, send to controller")
                fp_goal = FollowPathGoal()
                fp_goal.path = result.path
