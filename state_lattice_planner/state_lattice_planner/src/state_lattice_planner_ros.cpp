...
void StateLatticePlannerROS::process(void)
{
    ...
    while(ros::ok()){
        ...
        std::cout << "goal:" << local_goal_subscribed << " map:" << local_map_updated <<
                     " odom:" << odom_updated << " trans:" << goal_transformed << std::endl;
        if(local_goal_subscribed && local_map_updated && odom_updated && goal_transformed){
            ...
            if(goal.segment(0, 2).norm() < 0.1){ // 离目标距离小于 0.1 不需要生成路径
                generated = false;
